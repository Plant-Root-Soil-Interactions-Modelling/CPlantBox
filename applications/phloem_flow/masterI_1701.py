...
def runSim(directoryN_,doVTP, verbosebase,
          ...
          leafAsIAASource_ = True, limLenActive_ = 0.9):
    outcondition = 0
    useCWGr = True
    ...
    rho_org = [[1.34,1.34,1.34,1.34],[1.44*CarbonCost,1.44*CarbonCost],[1.56,1.56,1.56,1.56]]#g C/gDW?
    ...
    grRate = [[4.,2.,1.0,4.],[1.8*GrRatio,1.8*GrRatioLats],[3.,3.,3.,3.]]
...
#     #volOrg2_type = volOrgini2_type
    ...
    while  simDuration < simMax:#
        ...
        expandedLeaf = False
        if __name__ == '__main__':
            print("leaf",nodeD,leafArea,maxLeafArea,np.array([org.parentLinkingNode for org in leaves]) +1)
            print("leaf areas", np.array([org.getLength(True) * org.getParameter("Width_blade") for org in leaves]),np.array([org.getParameter("k") * org.getParameter("Width_blade")*limLenActive_ for org in leaves]),)
        ...
        toKeep = np.array([org.getParameter("subType") <= 2 for org in orgs])
        orgs = np.array(orgs)[toKeep]
        mainStem = r.plant.getOrgans(3, False)[0]
        temp_time = time.time()
        ...
        if(not r.burnInTime) and (doDict or ((dtreal >= dt_write) and (dtsimsim >= dtSIM_write) )):
            ...
            write_file_array("InAuxinAll", np.array(r.AuxinSource)* r.auxin_P * dt)
            ...
            stemNodes = np.array(mainStem.getNodeIds())
  ...
  "C_Auxin","AuxinSource","Delta_JA_ST","C_AuxinOut","JAuxin_ST2",
     "activePhloem","activeAuxin","Ag4Phloem","isRootTip","C_meso"
                                "QExud", "QGr", "QRm",
                                "CExud", "CGr", "CRm",
...
if __name__ == '__main__':
    ...
    PRate_ = 6.8e-3, PRBA = 0,  PRBD=0,PRBr = 0., thresholdAux = 0, 
     ...
     thresholdSuc = thresholdSuc_, 
           GrRatio =10, GrRatioLats =5,  CarbonCost=2,GrRatioLeaf_=10,CarbonCostLeaf_=1/10,GrRatioRoot_=1,CarbonCostRoot_=1/100,
...
def runAllSim(i, arg1,arg2):
    ...
    PRate_ = 6.8e-3, PRBA = 0,  PRBD=0,PRBr = 0., thresholdAux = 0, 
     ...
     Qmax_ = Qmax__, Klight = 0.,
     thresholdSuc = thresholdSuc_, 
           GrRatio = GrRatio_, GrRatioLats =GrRatioLats_,  CarbonCost=2,GrRatioLeaf_=GrRatioLeaf,CarbonCostLeaf_=1/10,GrRatioRoot_=1,CarbonCostRoot_=1/100,
