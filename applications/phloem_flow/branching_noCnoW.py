...
pl.simulate(100, True)#, "outputpm15.txt")#simDuration
#raise Exception
...
while simDuration < 50:#simMax:
    ...
    ana = pb.SegmentAnalyser(pl.mappedSegments())
    ana.write("results"+directoryN+"15pm_"+ str(ö) +".vtp", ["organType","subType"])
...
# print("leaf")
for p in pl.getOrgans(pb.leaf):
     print(len(pl.getOrgans(pb.leaf)), p.getId(), p.getLength(),p.getParameter("surface_th"),(p.getParameter("surface_th")/np.pi)**0.5)
#     #rlp_geom = p.
