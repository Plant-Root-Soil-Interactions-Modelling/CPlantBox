...
def runSim(directoryN_,doVTP, verbosebase,
           ...
           doPrint ,doDict,auxin_D = 0.):#start_time = 0.,doPrint =True,auxin_D = 0.
    ...
    while  simDuration < simMax:#
        ...
        errorAuxin   = sum(Q_Auxin_stem) + sum(Q_Auxin_other) - sum(Q_AuxinInit)- InAuxin +sum(OutAuxin) + sum(AuxinDecap)
        ...
        if verbosebase :
            ...
            print("Error in Aux_balance:\n\tabs (mmol) {:5.2e}\trel (-) {:5.2e}".format(errorAuxin, div0f(errorAuxin,Q_Auxin_stem, 1.)))
        ...
        if not r.burnInTime:
            ...
            ststdistFromParentBase = distFromParentBase[(ot_orgs == 3)* (st_orgs == 2)]#* activatedaux]
            ststlactivatedaux = activatedaux[(ot_orgs == 3)* (st_orgs == 2) ]#* activatedaux]
            ststlengthth_org = lengthth_org[(ot_orgs == 3)* (st_orgs == 2) ]#* activatedaux]
        ...
        if doVTP:
            ...
            ana.write("results"+directoryN+"plotplant_"+dir4allResults+"_"+ str(ö) +".vtp", ["CST", "fluxes","psi_Xyl",
  "C_Auxin","AuxinSource","Delta_JA_ST","C_AuxinOut","JAuxin_ST2",
     "activePhloem","activeAuxin","Agn4Phloem","isRootTip",
        ...
        if r.burnInTime :
            ...
            mainStemAux_CV = (np.std(mainStemAux[1:])**2)*len(mainStemAux[1:])
            mainStemAux_mean = np.mean(mainStemAux[1:])
        else :
            simDuration += dt
            
        if (mainStemAux_CV< 0.01) and (mainStemAux_mean > 1) and ( r.burnInTime):#(day4burnin <= 0) :
            r.burnInTime = False
            
            r.canStartActivating = True
            if UseRatiothresholdAux:
