...
def weather(simDuration):
    ...
    TairC_ = 25#Tmin + (Tmax - Tmin) * coefhours
    ...
    RH = 0.60#qair2rh(specificHumidity, es, Pair)
...
def setKrKx_xylem(TairC, RH): #inC
    ...
    kz_l  = VascBundle_leaf *(rad_x_l_1 + rad_x_l_2)    *np.pi /(mu * 8)#  *100
    kz_s  = VascBundle_stem *(rad_x_s_1 + rad_x_s_2)    *np.pi /(mu * 8)# *100
    kz_r0 = VascBundle_root * rad_x_r0_1                *np.pi /(mu * 8) # *100
    kz_r1 = VascBundle_root *(rad_x_r12_1 + rad_x_r12_2)*np.pi /(mu * 8) #*100
    kz_r2 = VascBundle_root *(rad_x_r12_1 + rad_x_r12_2)*np.pi /(mu * 8) # *100
    kz_r3 = VascBundle_root * rad_x_r3_1                *np.pi /(mu * 8)# *100# 4.32e-1

    #radial conductivity [1/day],
    kr_l  = 3.83e-4 * hPa2cm#*100# init: 3.83e-4 cm/d/hPa
    kr_s  = 0.#1.e-20  * hPa2cm # set to almost 0
    kr_r0 = 6.37e-5 * hPa2cm #*100
    kr_r1 = 7.9e-5  * hPa2cm #*100
    kr_r2 = 7.9e-5  * hPa2cm # *100
    kr_r3 = 6.8e-5  * hPa2cm #*100
    l_kr = 100#0.8 #cm
...
Qs = np.array([100e-6,600e-6]) # 580e-6, mol quanta m-2 s-1 light
