...
def setKrKx_phloem(): #inC
    ...
    a_ST = np.array([np.array(xi) for xi in a_ST])*10
    ...
    r.setKx_st([[kz_r0,kz_r12,kz_r12,kz_r0],[kz_s,kz_s, kz_s,kz_s],[kr_l,kz_l]])
...
name = "UQ_simple"#"morning_glory_UQ"#
...
grRate = np.array([np.array(xi) for xi in grRate])*10
...
print(org_)
print(org_.getId())
print( org_.getLength(),org_.getLength(org_.getNumberOfNodes()-2), org_.getNodeId(org_.getNumberOfNodes()-2), org_.getNodeId(org_.getNumberOfNodes()-1))
r.node_Decapitate = np.array([org_.getNodeId(org_.getNumberOfNodes()-2)])
print(r.node_Decapitate)
#raise Exception
