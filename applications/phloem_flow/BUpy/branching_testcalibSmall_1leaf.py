...
def setKrKx_phloem(): #inC
    ...
    r.setKx_st([[kz_r0,kz_r12,kz_r12,kz_r0],[kz_l,kz_l, kz_l,kz_l],[kr_l,kz_l]])
    print([[kz_r0,kz_r12,kz_r12,kz_r0],[kz_l,kz_l, kz_l,kz_l],[kz_l,kz_l]], kz_s)
    #raise Exception
...
name = "UQ_1leaf_bud"#"morning_glory_UQ"#
...
#grRate = [[4.,2.,1.0,4.],[8.,0.00000001,2.],[2.,2.]]
grRate = [[4.,2.,1.0,4.],[80.,0.00000001,2.],[1.,1.]]
...
while simDuration < simMax: 
    ...
    print("mumol CO2 m-2 s-1:\nAn {:5.2e}, Rd {:5.2e}, Vc {:5.2e}, Vj {:5.2e}, gco2 {:5.2e}".format(
            np.mean(np.array(r.An)[idleafBlade])*1e6, 
            r.Rd*1e6,
            np.mean(np.array(r.Vc)[idleafBlade])*1e6,
            np.mean(np.array(r.Vj)[idleafBlade])*1e6,
            np.mean(np.array(r.gco2)[idleafBlade])
            ))
    print("mol mol-1, [-]:\ncics {:5.2e}, ci {:5.2e}, deltagco2 {:5.2e}, fw {:5.2e}".format(
            np.mean(np.array(r.ci)[idleafBlade])/r.cs, 
            np.mean(np.array(r.ci)[idleafBlade]),
            np.mean(np.array(r.deltagco2)[idleafBlade]),
            np.mean(np.array(r.fw)[idleafBlade])
            ))
