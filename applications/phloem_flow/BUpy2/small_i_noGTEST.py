...
def runSim(directoryN_,PRate_ = 4, threshold = 0.8, doVTP = False, 
    ...
    def write_file_array(name, data):
        name2 = 'results'+ directoryN+ name+ "_"+ strPRate_ + "_"+strTh+"_"+strDecap+ '.txt'
    ...
    pl.simulate(8, False)#, "outputpm15.txt")
    simDuration = 8
    ...
    r.auxin_alpha = 0.02 #5#e-6# 2.4e-3
    ...
    if doVTP:
        ...
        activePhloemv_ = [np.full(len(org.getNodeIds()[1:]),org.activePhloem) for org in orgs]
        ...
        activeAuxinv_ = [np.full(len(org.getNodeIds()[1:]),org.activeAuxin) for org in orgs]
    ...
    day4burnin = 2
    r.burnInTime = True
    ##change gr rate of buds
    Allstems = r.plant.getOrgans(3, True)
    for stm in Allstems:
        if stm.getParameter("subType") == 2 :
            stm.
    while  simDuration < 30:#ö < 10: #
        ...
        errorAuxin   = sum(Q_Auxin) - sum(Q_AuxinInit)- InAuxin +sum(OutAuxin)
        ...
        if verbosebase :
            print("\n\n\n\t\tat ", int(np.floor(simDuration)),"d", int((simDuration%1)*24),"h",  round(r.Qlight *1e6),"mumol m-2 s-1",int(np.floor(r.burnInTime)),"d" )
            ...
            print("Auxin (mmol):\n\tPl {:5.2e}\tInit {:5.2e}\tIn {:5.2e}\n\tOut {:5.2e}".format(sum(Q_Auxin), sum(Q_AuxinInit), InAuxin , sum(OutAuxin))) 
            print("Error in Aux_balance:\n\tabs (mmol) {:5.2e}\trel (-) {:5.2e}".format(errorAuxin, div0f(errorAuxin,sum(Q_Auxin), 1.)))
        ...
        if abs(error) > 1e-3:
            ...
            raise Exception    
        if abs(div0f(errorAuxin,sum(Q_Auxin), 1.)) > 1e-3:
        ...
        if doVTP:
            ...
            print(len(activePhloemv),len(apv),len(apvminus),len(C_ST_p),r.plant.getNumberOfNodes())
  ...
  "C_Auxin","AuxinSource","Delta_JA_ST","C_AuxinOut","JAuxin_ST2",
     "activePhloem","activeAuxin","Agn4Phloem",
                                "QExud", "QGr", "QRm",
                                "CExud", "CGr", "CRm",
        ...
        if (ö>=2) and (not r.burnInTime):
            r.canStartActivating = True
            simDuration += dt
        else :
            day4burnin -= dt
            
        if (day4burnin <= 0) and ( r.burnInTime):
            r.burnInTime = False
    ...
    if not os.path.exists(results_dir):
       ...
       for item in test:
            try:
                ...
                pass
    #raise Exception
    runSim(directoryN_=directoryN,PRate_ = 10, threshold = 0.4, doVTP = True, 
