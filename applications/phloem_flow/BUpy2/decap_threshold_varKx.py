...
directoryN = "/decap_threshold_varKx/"
...
def delete_verbose():
    ...
    for item in test:
        ...
        if item.startswith("/results"+directoryN+"prints"):
            os.remove(os.path.join(dir_name2, item))
    test = os.listdir(os.path.join(home_dir,"results"+directoryN))
    for item in test:
        os.remove("results"+directoryN+item)
delete_verbose()
...
def setKrKx_phloem(): #inC
    ...
    a_STend    = [[0.00039/2,0.00035/2,0.00035/2,0.00039/2],[0.00019/2,0.00019/2],[0.00025/2]]
    a_STend = np.array([np.array(xi) for xi in a_STend], dtype=object)
    ...
    rad_s_lend   = numL  * (a_STend[2][0] ** 4)# * 2; rad_x_l_2   = (0.0005 **4) * 2   
    rad_s_send   = numS  * (a_STend[1][0] ** 4) #* 3; rad_x_s_2   = (0.0008 **4) * 1     
    rad_s_r0end  = numr0 * (a_STend[0][0] ** 4) #* 4    
    rad_s_r12end = numr1 * (a_STend[0][1] ** 4) #* 4; rad_x_r12_2 = (0.00087**4) * 1
    rad_s_r3end  = numr3 * (a_STend[0][2] ** 4) #* 1      
    ...
    kz_lend   = VascBundle_leaf * rad_s_lend   * np.pi /8 * beta  
    kz_send   = VascBundle_stem * rad_s_send   * np.pi /8 * beta
    kz_r0end  = VascBundle_root * rad_s_r0end  * np.pi /8 * beta
    kz_r12end = VascBundle_root * rad_s_r12end * np.pi /8 * beta
    kz_r3end  = VascBundle_root * rad_s_r3end  * np.pi /8 * beta
    ...
    #-0.93 per m
    r.setKx_st_table([[[kz_r0],[kz_r12],[kz_r12],[kz_r0]],[[kz_lend,kz_l],[kz_lend,kz_l], [kz_l]],[[kz_l]]],
                     [[[0],[0],[0],[0]],[[0,20],[0,20], [20]],[[0]]]
                        )
    #print([[[kz_r0],[kz_r12],[kz_r12],[kz_r0]],[[kz_lend,kz_l],[kz_lend,kz_l], [kz_lend,kz_l]],[[kz_lend,kz_l]]])
    #raise Exception
    ...
    Across_s_lend   = numL*VascBundle_leaf *(a_STend[2][0]**2)*np.pi# (0.00025 **2)# * 2; rad_x_l_2   = (0.0005 **4) * 2   
    Across_s_send   = numS *VascBundle_stem * (a_STend[1][0]**2)*np.pi#(0.00019 **2) #* 3; rad_x_s_2   = (0.0008 **4) * 1     
    Across_s_r0end  = numr0 *VascBundle_root * (a_STend[0][0]**2)*np.pi#(0.00039 **2) #* 4    
    Across_s_r12end = numr1*VascBundle_root * (a_STend[0][1]**2)*np.pi#(0.00035**2) #* 4; rad_x_r12_2 = (0.00087**4) * 1
    Across_s_r3end  =  numr3 *VascBundle_root *(a_STend[0][2]**2)*np.pi# (0.00068**2) #* 1    
    ...
    #-14% per m
    r.setAcross_st_table([[[Across_s_r0],[Across_s_r12],[Across_s_r12],[Across_s_r0]],
                        [[Across_s_send,Across_s_s], [Across_s_send,Across_s_s],[Across_s_s]],
                        [[Across_s_l]]],
...
r.k_meso = 1e-3#1e-4
r.setKrm2([[0]])#2e-4
r.setKrm1([[3e-03]])#3e-03#([[2.5e-2]])
#r.setRhoSucrose([[0.51],[0.65],[0.56]])
rho_org = [[1.34],[1.44],[1.56]]
density = 0.17
rho_org = np.array([np.array(xi) for xi in rho_org])*density#*10
r.setRhoSucrose(rho_org)
#r.setRmax_st([[14.4,9.0,6.0,14.4],[5.,5.,5.,5.],[15.,15.,15.,15.]])#*6 for roots, *1 for stem, *24/14*1.5 for leaves
#r.setRmax_st([[12,9.0,6.0,12],[5.,5.],[15.]])
#grRate = [[4.,2.,1.0,4.],[2.,0.01,2.],[2.,2.]]
...
def killChildren(orgToKill):
    ...
    if orgToKill.getNumberOfChildren() > 0:
        ...
        print("temp2 ",toFill)
    return toFill
def killChildren_(orgToKill):
    toFill = np.array(orgToKill.getNodeIds())
    if orgToKill.getNumberOfChildren() > 0:
        toFill_ = np.array([killChildren(orgToKill.getChild(ni)) for ni in range(orgToKill.getNumberOfChildren())],dtype=object).flatten()
        toFill = np.concatenate((toFill, toFill_))
    return toFill
...
print(kid_pni,selectKids)
k_ =[killChildren(org_.getChild(ni)) for ni in selectKids]
toFill_ = [item for sublist in k_ for item in sublist]
print("killChildren ",toFill_)
toKil  = np.unique(toFill_)
print("end kill", toKil)


# kid = org_.getChild(org_.getNumberOfChildren()-1)
# kid2 = np.array([kid.getChild(ni).getNodeIds() for ni in range(kid.getNumberOfChildren())]).flatten()

# print(org_)
# print(org_.getId())
# print(kid)
# print(kid.getId())
# print(kid.getNodeIds())
# print(kid2)
# yy = np.array([org_.getNodeId(org_.getNumberOfNodes()-2), org_.getNodeId(org_.getNumberOfNodes()-1)])

# allNodesDecapitated = np.concatenate((yy,kid.getNodeIds(),kid2))
# print(allNodesDecapitated)
# print(yy,kid.getNodeIds(),kid2)

# allnodes = r.segments

# raise Exception
# print( org_.getLength(),org_.getLength(False),org_.getLength(org_.getNumberOfNodes()-2), org_.getNodeId(org_.getNumberOfNodes()-2), org_.getNodeId(org_.getNumberOfNodes()-1))
# lengthatnodes = [org_.getLength(ni) for ni in range(org_.getNumberOfNodes())]
# print(lengthatnodes, org_.getLength(7))
# #raise Exception
print(r.plant.node_Decapitate)
r.plant.node_Decapitate = toKil#np.array([org_.getNodeId(org_.getNumberOfNodes()-2), org_.getNodeId(org_.getNumberOfNodes()-1)])
#r.plant.node_Decapitate = np.array([org_.getNodeId(ni) for ni in range(9, org_.getNumberOfNodes())])
print(r.plant.node_Decapitate)
#raise Exception
r.activeAtThreshold = True
r.CSTthreshold = 0.8#[[-1],[-1,0.3,-1],[-1]]
orgs_stems = r.plant.getOrgans(3, True)  
while  simDuration < simMax: #ö< 5:
    ...
    r_ST_ref_p = r_ST_ref
    r_ST_ref_p[abs(r_ST_ref_p) < cutoff] = 0
    ...
    ana.addData("r_ST_ref", r_ST_ref_p)
    ...
    ana.write("results"+directoryN+"15pm_"+ str(ö) +".vtp", ["r_ST_ref","CST", "fluxes","psi_Xyl",
                        "QExud", "QGr", "QRm",
                        "CExud", "CGr", "CRm",
    ...
    #print(C_ST[r.node_Decapitate[0]])
    #raise Exception
