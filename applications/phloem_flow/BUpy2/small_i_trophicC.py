...
def runSim(directoryN_,PRate_ = 4, threshold = 0.8, Qmax_ = 1000e-6,
           doVTP = False, 
            nodeD = 3, thread = 0,  verbosebase = False, testTime=7):
    ...
    def write_file_array(name, data):
        name2 = 'results'+ directoryN+ name+ "_"+ strPRate_ + "_"+ strQ + "_"+strTh+"_"+strDecap+ '.txt'
    ...
    r.auxin_D = 0.5 #e-6#muM /d  #e-6#3e-7
    r.auxin_P = PRate_ #muM /d  #*1e-6# 6e-7
    r.auxin_alpha = 0.00245669#0.02 #5#e-6# 2.4e-3
    r.initValAuxin = 0 #0.2
    r.deleteAtRootTip = True
    r.plant.useCWGr = True
    InAuxin = 0
    ...
    while  simDuration < simMax:#
        ...
        filename = "results"+ directoryN +"inPM_"+str(ö)+ "_"+ strPRate_ + "_"+ strQ + "_"+strTh+"_"+strDecap+".txt"
        print("startpm")
        ...
        if verbosebase :
            ...
            print("Auxin (mmol):\n\tPl {:5.2e}\tInit {:5.2e}\tIn {:5.2e}".format(sum(Q_Auxin), sum(Q_AuxinInit), InAuxin )) 
            print("\tOut {:5.2e}\tAuxCut {:5.2e}".format(sum(OutAuxin), sum(AuxinDecap))) 
            print("Error in Aux_balance:\n\tabs (mmol) {:5.2e}\trel (-) {:5.2e}".format(errorAuxin, div0f(errorAuxin,InAuxin, 1.)))
        ...
        if not r.burnInTime:
            ...
            write_file_float("numLNodes",write_file_float)
        ...
        if doVTP:
            ...
            ana.write("results"+directoryN+"plotplant_"+strPRate_ + "_"+ strQ + "_"+strTh+"_"+strDecap+"_"+ str(ö) +".vtp", ["CST", "fluxes","psi_Xyl",
     ...
     "activePhloem","activeAuxin","Agn4Phloem","isRootTip",
            ...
            if(False):#(simDuration > 0):#if(simDuration > (simMax -1)):
                vp.plot_plant(r.plant,p_name = [ "fluxes","Exud","Gr","Rm","xylem pressure (cm)"],
                                    ...
                                    filename = "results"+ directoryN +"plotplant_psi_"+strPRate_ + "_"+ strQ + "_"+strTh+"_"+strDecap+"_"+ str(ö),
                                    range_ = [300,1450])
                vp.plot_plant(r.plant,p_name = [ "fluxes","Exud","Gr","Rm","sucrose concentration (mmol/cm3)"],
                                    ...
                                    filename = "results"+ directoryN +"plotplant_suc_"+strPRate_ + "_"+ strQ + "_"+strTh+"_"+strDecap+"_"+ str(ö),
                                    range_ = [0,3])   
    ...
    if not os.path.exists(results_dir):
       ...
       for item in test:
            try:
                ...
                pass
    #raise Exception
    runSim(directoryN_=directoryN,PRate_ = 10, threshold = 40, doVTP = True, 
           nodeD = 4, thread = 0, verbosebase = True,testTime=7 )