...
def reconstruct_laterals(polylines, properties, base_polyline, snap_radius=0.5):
    ...
    for i, pi in enumerate(pp):  # iterate over roots in polylines
        if pi == -1:  # tap root
...
def analyze_zones(polylines, properties):
    ...
    n0 = 0
    nl0 = ii[0]
    lb = polyline_length(n0, nl0, polylines[0])

    ne = ii[-1]
    nle = len(polylines[0]) - 1
    la = polyline_length(ne, nle, polylines[0])
...
def create_age_delay(polylines, properties, time):
    """
    create root age 
    """
    pni = properties["parent-node"]
