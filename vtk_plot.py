...
def plot_plant(plant, p_name, render=True, interactiveImage=True):
    ...
    pd = segs_to_polydata(ana, 1.0, ["radius", "organType", "creationTime", p_name])  # poly data
    ...
    globalIdx_y = []
    leaves = plant.getOrgans(
        ot=pb.leaf
    )  # <--------- TODO DL: will only work for Plant, not for MappedSegments
    for l in leaves:
    ...
    if render:
        ren = render_window(
            [tube_plot_actor, actor],
            "plot_plant",
            color_bar,
            tube_plot_actor.GetBounds(),
            interactiveImage,
        )
        if interactiveImage:
            ren.Start()
...
def segs_to_polydata(
    rs, zoom_factor=1.0, param_names=["age", "radius", "type", "organTypecreationTime"]
):
    ...
    for n in param_names:
        ...
        if param.shape[0] == segs.shape[0]:
            ...
            print(
                "segs_to_polydata: Warning parameter " + n + " is sikpped because of wrong size",
                param.shape[0],
                "instead of",
                segs.shape[0],
            )
...
def create_lookup_table(tableIdx=15, numberOfColors=256):
    ...
    for i in range(0, numberOfColors):
        psi = (n - 1) * (
            float(i) / numberOfColors
        )  # float(i), float(numberOfColors - i - 1) to reverse
        i0 = np.floor(psi)
        theta = psi - i0
        col = (1 - theta) * np.array(lut_.GetTableValue(int(i0))) + theta * np.array(
            lut_.GetTableValue(int(i0) + 1)
        )
...
def plot_segments(
    pd, p_name: str, win_title: str = " ", render: bool = True, interactiveImage: bool = True
):
    """see plot roots"""
...
def plot_roots(
    ...
    win_title: str = "",
    render: bool = True,
    interactiveImage: bool = True,
    returnLut: bool = False,
):
    ...
    if isinstance(pd, pb.RootSystem):
        pd = segs_to_polydata(pd, 1.0, [p_name, "radius", "organType"])

    if isinstance(pd, pb.Plant):
        pd = segs_to_polydata(pd, 1.0, [p_name, "radius", "organType"])

    if isinstance(pd, pb.SegmentAnalyser):
        pd = segs_to_polydata(pd, 1.0, [p_name, "radius"])
...
def plot_plant_and_soil(
    ...
    pd = segs_to_polydata(ana, 1.0, [pname, "radius"])
    ...
    ren = render_window(
        meshActors, filename, [meshCBar, rootCBar], pd.GetBounds(), interactiveImage
    )
    if interactiveImage:
        ren.Start()
...
def plot_roots_and_mesh(
    ...
    pd = segs_to_polydata(ana, 1.0, ["radius", "subType", "creationTime", pname_root])
...
def write_plant(filename, plant, add_params=[]):
    ...
    pd = segs_to_polydata(
        plant, 1.0, params
    )  # TODO: DL this would work for a SegmentAnalyser (and there we could attach data),  maybe split leaf part
...
def plot_roots_and_soil_files(
    filename: str, pname_mesh: str, pname: str, path="results/", interactiveImage=True
):
    ...
    def create_soil_actors(self):
        if self.soil_data:
            if self.cuts:
                ...
                meshActor, meshCBar, grid = plot_mesh_yz(
                    self.soil, "pressure head", self.min, self.max, self.res
                )
...
def plot_roots_and_container(
    root_system,
    sdf,
    p_name="subType",
    title="Root System & Container",
    render=True,
    interactive=True,
):
