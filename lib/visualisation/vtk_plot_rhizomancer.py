...
def plot_roots(pd, p_name:str, win_title:str = "", render:bool = True, interactiveImage:bool = True, returnLut:bool = False):
    ...
    final_pd = tubeFilter.GetOutput()

    data_array = final_pd.GetCellData().GetAbstractArray(p_name)
    if data_array is None: # Fallback to point data if not in cell data
        data_array = final_pd.GetPointData().GetAbstractArray(p_name)
        
    lut = create_lookup_table()
    if data_array:
        data_range = data_array.GetRange()
        lut.SetTableRange(data_range)

    mapper = vtk.vtkPolyDataMapper()
    mapper.SetInputConnection(tubeFilter.GetOutputPort()) # Use original connection
    mapper.Update()
    mapper.ScalarVisibilityOn();
    mapper.SetScalarModeToUseCellFieldData()
    mapper.SelectColorArray(p_name)
    mapper.UseLookupTableScalarRangeOn()
    mapper.SetLookupTable(lut) # Apply the correctly scaled lookup table
    
    plantActor = vtk.vtkActor()
    plantActor.SetMapper(mapper)

    if render:
        ren = render_window(plantActor, win_title, None, pd.GetBounds(), interactiveImage) 
        if interactiveImage:
            ren.Start()

    if returnLut:
        return plantActor, lut 

    return plantActor, data_range
