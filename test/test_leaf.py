...
#         p0.createLeafGeometry(y, l, N)
    ...
    def leaf_example_rtp(self):
        ...
        p1.name, p1.subType, p1.lmax, p1.ln, p1.r, p1.dx = "lateral", 3, 25, 0, 2, 0.1
        ...
        self.leaf = pb.Leaf(self.plant, p0.subType, 0, self.parentleaf, 0)
    ...
    def test_constructors(self):
        ...
        leaf = pb.Leaf(1, param, True, True, 0., 0., pb.Vector3d(0, 0, -1), 0, False, 0)
        ...
        leaf2 = pb.Leaf(self.plant, self.p1.subType,0, leaf, 0)
        leaf.addChild(leaf2)
    ...
    def test_leaf_length_including_laterals(self):
        ...
        k = p.getK()
        meanln = self.leaf.getParameter('lnMean')
        et = np.zeros((p.nob()))
        l = 0
        et[0] = leafAge(max(p.la - meanln / 2.,0.) + p.lb + l, p.r, k)
        for i in range(0, p.nob() - 1):  # calculate lateral emergence times
            l += p.ln[i]
            et[i + 1] = leafAge(max(p.la - meanln / 2.,0.) + p.lb + l, p.r, k + 1e-12)
        ...
        for subDX in [1, 1000]:
            ...
            for t in times[1:]:
                ...
                nl = 0
                for ido, o in enumerate(organs):
                    nl += o.getParameter("length")
                    if ido > 0:        
                        self.assertAlmostEqual(et[ido-1], o.getParameter('creationTime'), 10, "numeric and analytic total lengths do not agree in time step " + str(i + 1))
    ...
    def test_dynamics(self):
        """ tests if leaves are always set to hasMoved """  #
        self.leaf_example_rtp()
        r = self.leaf
        r.simulate(.5, True)
        self.assertEqual(r.hasMoved(), True, "dynamics: node movement during first step")
        r.simulate(1e-1, True)
        self.assertEqual(r.hasMoved(), True, "dynamics: movement, but previous node at axial resolution")
        r.simulate(1e-1, True)
