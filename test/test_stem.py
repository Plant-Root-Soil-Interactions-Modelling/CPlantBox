...
class TestStem(unittest.TestCase):
    ...
    def stem_example_rtp(self, phytomereGrowth = "sequential"):
        ...
        paramS = srp.realize()
        self.seed = self.plant.getSeed()#pb.Seed(8, paramS, True, True, 0., 0., False, 0)
        #self.seed.setOrganism(self.plant)
        #self.plant.addOrgan(self.seed)
        #self.seed.addNode(pb.Vector3d(0, 0, -3), 0)  # there is no nullptr in Python
        ...
        print(self.seed.getNumberOfChildren())
        print(self.plant.getSeed().getNumberOfChildren(),self.plant.getSeed().organType(), self.plant.getSeed().getId())
        #raise Exception
    ...
    def stem_length_test(self, dt, l):
        ...
        for t in dt:
            ...
            nl2.append(sum(sd))
            print(sd, self.stem.getParameter("length"), self.stem.getParameter("lengthTh"))
        print(nl2)
        print(nl)
    ...
    def test_dynamics(self):
        ...
        print(stemLength_, r.getLength())
        #self.assertAlmostEqual(stemLength_, r.getLength(), 12,"example stem has wrong length")
        basalZoneLength = min(r.param().lb, stemLength_)
        print(r.param().lb, stemLength_)
        ...
        if(basalZoneLength < stemLength_):
            ...
            for o in range(0, latNum):
                ...
                print("parentNI",o, pni, r.getLength(int(pni)))
            for o in range(0, latNum):
                pni = r.getChild(o).getParameter("parentNI")
                PhytoEnd = r.getLength(int(pni))
                PhytoLength = PhytoEnd - PhytoStart
                print("PhytoEnd , PhytoStart",PhytoEnd , PhytoStart)
                #raise Exception
        ...
        print(basalZoneNodes ,PhytoNodes , apicalZoneNodes)
    ...
    def test_stem_length_including_laterals(self):
        ...
        l = 0
        et_init = stemAge(p.lb, p.r, k, p.delayNGStart, p.delayNGEnd, p.lb) #self.stem.calcAge(0)#
        ...
        for i in range(0, len(times)):
            ...
            print("len(organs) ",len(organs), self.stem.getNumberOfChildren())
            nl = []
            nlth = []
