...
class TestRoot(unittest.TestCase):
    ...
    def test_constructors(self):
        ...
        root = pb.Root(1, param, True, True, 0.0, 0.0, pb.Vector3d(0, 0, -1), 0, False, 0)
    ...
    def test_root_length(self):
        ...
        self.root_example_rrp()
        times = np.array([0.0, 7.0, 15.0, 30.0, 60.0])
        dt = np.diff(times)
    ...
    def test_root_length_including_laterals(self):
        ...
        et = np.zeros((p.nob()))
        l = 0
        et[0] = rootAge(p.la - rp.ln / 2.0 + p.lb + l, p.r, k)
        for i in range(0, p.nob() - 1):  # calculate lateral emergence times
            l += p.ln[i]
            et[i + 1] = rootAge(p.la - rp.ln / 2.0 + p.lb + l, p.r, k)
    ...
    def root_example_rrp2(self):
        ...
        p0 = pb.RootRandomParameter(self.plant)
        p0.name, p0.subType, p0.la, p0.lb, p0.lmax, p0.ln, p0.lnk, p0.r, p0.dx, p0.dxMin = "taproot", 1, 0.95, 0.8, 10.0, 1.05, 0.01, 0.8, 0.25, 0.2
        p0.successor = [[2]]
        p0.successorP = [[1.0]]
        p1 = pb.RootRandomParameter(self.plant)
        p1.name, p1.subType, p1.lmax, p1.r, p1.dx = "lateral", 2, 2.0, 2.0, 2.0
        ...
        param0.la, param0.lb = 0, 0  # its important parent has zero length, otherwise creation times are messed up
        parentroot = pb.Root(1, param0, True, True, 0.0, 0.0, pb.Vector3d(0, 0, -1), 0, 0, False, 0)  # takes ownership of param0
        parentroot.setOrganism(self.plant)
    ...
    def root_example_rtp2(self, delay_definition=1):
        ...
        p0 = pb.RootRandomParameter(self.plant)
        self.lmax_th = 100
        p0.name, p0.subType, p0.la, p0.lb, p0.lmax, p0.ln, p0.r, p0.dx, p0.dxMin = "main", 1, 10.0, 10.0, self.lmax_th, 1.0, 1.5, 1, 0.5
        p0.ldelay = 1.0
        p0.successor = [[5]]
        p0.successorP = [[1.0]]

        p1 = pb.RootRandomParameter(self.plant)
        p1.name, p1.subType, p1.lmax, p1.r, p1.dx, p1.dxMin = "lateral", 5, 5.0, 2.0, 1, 0.5
        p1.ldelay = 2.0
        self.p0, self.p1 = p0, p1  # needed at later point
        self.plant.setOrganRandomParameter(p0)  # the organism manages the type parameters and takes ownership
        self.plant.setOrganRandomParameter(p1)
        ...
        srp = pb.SeedRandomParameter(self.plant)
        srp.delayDefinitionRoot = delay_definition  # organ carries the delay of its laterals
        self.plant.setOrganRandomParameter(srp)
        ...
        parentroot = pb.Root(1, param0, True, True, 0.0, 0.0, self.partialiheading, 0, False, 0)  # takes ownership of param0
        parentroot.setOrganism(self.plant)
    ...
    def test_new_delay_types(self):
        ...
        effectiveLa = max(self.root.getParameter("la") - meanLn / 2, 0.0)  # effective apical distance, observed apical distance is in la-ln/2, la+ln/2
