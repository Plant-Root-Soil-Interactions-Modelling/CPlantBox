...
"""
One click install script for dumux-rosi (macOS version)
"""
...
if not os.path.exists("dumux-rosi"):
    subprocess.run(['git', 'clone', '--depth', '1', '-b', 'master',
                    'https://github.com/Plant-Root-Soil-Interactions-Modelling/dumux-rosi.git'])
...
if "VIRTUAL_ENV" in os.environ:
    ...
    rosi_path = os.path.join(
        os.environ["VIRTUAL_ENV"],
        f"lib/python{pyver}/site-packages/rosi"
    )
...
lines = [f'export DYLD_LIBRARY_PATH="{plantbox_path}:$DYLD_LIBRARY_PATH"\n',
	f'export DYLD_LIBRARY_PATH="{rosi_path}:$DYLD_LIBRARY_PATH"\n',
...
Test it by running\ncd CPlantBox/tutorial/chapter7_coupled\npython3 example7_2_coupling.py
