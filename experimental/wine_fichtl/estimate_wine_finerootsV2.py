...
def checkData(year):
    ...
    properties => 'parent-poly', 'parent-node', 'order', 'length'
    """
    ...
    root_lengths =  np.array([x for sublist in root_lengths for x in sublist])
    root_order = [np.array(data.rsmls[i].properties["order"]) for i in range(0, len(file_names))]
    root_order =  np.array([x for sublist in root_order for x in sublist])
    root_age = rootAge(root_lengths, 790 * 3.28 * 10e-5 * 1225,790)
    try:
        root_diameter = [data.rsmls[i].functions["diameter"] for i in range(0, len(file_names))]
        root_diameter =  [x for sublist in root_diameter for x in sublist]
        root_diameter =  np.array([np.mean(x) for x in root_diameter])
    except:
        root_diameter =  np.ones(root_order.shape)

    max_type = max(root_order)
    #print(data.rsmls[0].functions["diameter"], [np.mean(ff) for ff in data.rsmls[0].functions["diameter"]])

    # fig, ax = plt.subplots(1, 1, figsize = (9, 8))
    # for order in range(max_type + 1) :
        # plt.scatter(root_diameter[root_order == order], 
                        # root_lengths[root_order == order], label=order)
    # plt.legend()
    # #plt.ylim(top=6)
    # #plt.xlim(right=0.093)
    # plt.show()

    if year == 1:
        fine_roots = (root_lengths < 5) & (root_diameter < 0.093) 
    else:
        fine_roots = (root_lengths < 5) 
...
# def checkData(year):
    ...
    # for i in range(0, len(file_names)):
        # root_lengths = np.array(data.rsmls[i].properties["length"])
        # root_order = np.array(data.rsmls[i].properties["order"])
        # root_diameter = data.rsmls[i].functions["diameter"]
        # root_diameter =  np.array([np.mean(x) for x in root_diameter])
...
fig, ax = plt.subplots(1, 1, figsize = (9, 8))
for ff in range(len(root_order)):        
    
    #plt.scatter([np.mean(ff)*10 for ff in data.rsmls[ff].functions["diameter"]] , root_lengths[ff], c=root_order[ff])#c = root_order[ff], label=file_names[ff])
    plt.scatter(root_order[ff],[np.mean(ff)/2 for ff in data.rsmls[ff].functions["diameter"]], label=file_names[ff])
...
# fig, ax = plt.subplots(1, 1, figsize = (9, 8))
# for ff in range(len(root_order)):
    # plt.scatter(root_order[ff], root_lengths[ff], label=file_names[ff])
# plt.legend()
