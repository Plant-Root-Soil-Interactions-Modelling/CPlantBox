...
def launchUQR(directoryN,simInit, condition,forPlants):
    ...
    volOrg2 = np.array([org.orgVolume(-1,False) for org in orgs_all2]) 
    ot_orgs2 = np.array([org.organType() for org in orgs_all2])
    st_orgs2 = np.array([org.getParameter("subType") for org in orgs_all2])
    ...
    while simDuration < simMax: 
        ...
        """ dumux """    
        fluxesSoil1 = r.soilFluxes(simDuration, r.psiXyl, sx, approx=False)
        fluxesSoil2 = r2.soilFluxes(simDuration, r.psiXyl, sx, approx=False)
        
        fluxesSoil = {key: fluxesSoil1.get(key, 0) + fluxesSoil2.get(key, 0)
                  for key in set(fluxesSoil1) | set(fluxesSoil2)}
        ...
        r2.startPM(startphloem, endphloem, stepphloem, ( weatherX["TairC"]  +273.15) , verbose_phloem, filename)
        ...
        Q_Gr2_i       = Q_Gr_i[np.where(ot_4phloem==2)[0]]
        ...
        ana2 = pb.SegmentAnalyser(r2.plant.mappedSegments())
        ...
        ana2.addData("Fpsi", np.array(r2.Fpsi))
        ...
        ana.write("results"+directoryN+"p1p_"+ str(ö) +".vtp", ["CST", "fluxes","psi_Xyl",
                            "QExud", "QGr", "QRm",
                            "CExud", "CGr", "CRm",
    ...
    #     fluxes_p = np.insert(fluxes_p,0,0)# "[sucrose]",
        if False: #(simDuration > 0):# (simMax -1)):
        ...
        write_file_array("volOrg2",  volOrg2) #with epsilon
        write_file_array("ot_orgs2", ot_orgs2)
        write_file_array("st_orgs2", st_orgs2)
