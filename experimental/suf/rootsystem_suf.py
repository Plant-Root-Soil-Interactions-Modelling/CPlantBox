...
#     l2_.append(ana.getSummed("length"))
# plt.ylabel("root system length $[cm]$")
# plt.plot(time, l_, "r")
# plt.plot(time, l2_, "g")
# plt.show()    

fig, (ax1, ax2, ax3) = plt.subplots(3, figsize=(7, 20))
...
for ii, kx in enumerate(kx_):
    ...
    # artificial shoot
    kr0 = np.array([[-1e4, 0.], [0, 1.e-12], [10, 1.e-12]])  # always very small 
    kx0 = 1.  # always big
    ...
    rs.getRootSystemParameter().seedPos.z = -3  # -0.01
    rs.initialize()
    rs.simulate(simtime, True)  # simulate all, then use age dependent conductivities for predefined growth    
...
#     vp.plot_roots(rs, "creationTime")
    ...
    kr4 = kr1  # basal
    kr5 = kr1  # shoot borne
    r.setKrTables([kr1[:, 1], kr1[:, 1], kr2[:, 1], kr3[:, 1], kr4[:, 1], kr5[:, 1]], [kr1[:, 0], kr1[:, 0], kr2[:, 0], kr3[:, 0], kr4[:, 0], kr5[:, 0]])
...
""" Root system length """
ax3.set_ylabel("root system length $[cm]$")
ax3.plot(time, l_)
plt.show()
