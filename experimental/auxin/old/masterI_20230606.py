...
def runSim(directoryN_,doVTP, verbosebase,
          ...
          leafAsIAASource_ = True, limLenActive_ = 0.9, growthUpToNode = 9, 
           C_targMesophyll = 100, k_S_Mesophyll = 5/24):
    outcondition = 0
    useCWGr = True
    ...
    allInputsData = np.array([directoryN_,doVTP, verbosebase,
           ...
           doPrint ,doDict, auxin_D ,kss,kaa, 
           fileparam  , doMemAux,
          doDiffLights , Klight , BerthLim ,PRBA,  PRBD,C_targMesophyll, k_S_Mesophyll])
    ...
    r.C_targMesophyll = C_targMesophyll
    r.k_S_Mesophyll = k_S_Mesophyll
    ...
    while  simDuration < simMax:#
        ...
        error   = sum(Q_ST +Q_meso + Q_out + Q_STLost + Q_MesoLost + Q_S_Mesophyll)- Q_in - sum(Q_ST_init)  - sum(Q_meso_init) - sum(manualAddST) -sum(manualAddMeso)
...
if __name__ == '__main__':
    ...
    from DBrepExp_WT06062023 import toTry
    ...
    #doCondition = doCondition_
           ...
           C_targMesophyll = 2e-5, k_S_Mesophyll = 1
          )
    end_time_ = time.time()
