...
dumuxDir = "../../../../../DUMUX/dumux-rosi/python/coupled/"
sys.path.append(dumuxDir+"../build-cmake/cpp/python_binding/") # dumux python binding
sys.path.append(dumuxDir+"../../build-cmake/cpp/python_binding/")
sys.path.append(dumuxDir+"../modules/") # python wrappers 
sys.path.append(dumuxDir) # python wrappers 
sys.path.append(dumuxDir+"../../../") # python wrappers 
sys.path.append(dumuxDir+"../../") # python wrappers 
sys.path.append("../../../../../DUMUX/dumux-rosi/build-cmake/cpp/python_binding/")
#import matplotlib
...
def launchUQR(directoryN,simInit,simStartSim, condition,csChoise,spellDuration, simEnd):
    ...
    r.k_S_Mesophyll = 0#5/24 #daudet2002 
    r.k_S_ST = 0#5/24 #daudet2002 
    ...
    while simDuration < simMax: 
        ...
        dynamic_soil = False#((simDuration > simStartSim) and (simDuration <= simStartSim +spellDuration))
        ...
        Q_S_ST   = np.array(r.Q_out[(Nt*9):(Nt*10)])
        Q_S_Mesophyll   = np.array(r.Q_out[(Nt*10):(Nt*11)])
        ...
        error   = sum(Q_ST +Q_Par+ Q_meso + Q_out +Q_S_ST +Q_S_Mesophyll)- Q_in - sum(Q_ST_init)  - sum(Q_meso_init)
        ...
        Q_S_Mesophyll_i = Q_S_Mesophyll - Q_S_mesobu
        ...
        if verbose :
            ...
            print('\tS_ST   {:.2e}\tS_meso   {:.2e}'.format(sum(Q_S_ST), sum(Q_S_Mesophyll)))
        ...
        C_S_ST_i_p = Q_S_ST_i/volST
        C_S_ST_i_p[abs(C_S_ST_i_p) < cutoff] = 0
        C_S_Mesophyll_i_p = Q_S_Mesophyll_i/volMeso
        C_S_Mesophyll_i_p[abs(C_S_Mesophyll_i_p) < cutoff] = 0
        
        C_S_ST_p = Q_S_ST/volST
        C_S_ST_p[abs(C_S_ST_p) < cutoff] = 0
        C_S_Mesophyll_p = Q_S_Mesophyll/volMeso
        C_S_Mesophyll_p[abs(C_S_Mesophyll_p) < cutoff] = 0
        ...
        ana.addData("C_S_ST_i_p", C_S_ST_i_p)  # cut off for vizualisation
        ana.addData("C_S_Mesophyll_i_p", C_S_Mesophyll_i_p)  # cut off for vizualisation
        ana.addData("C_S_ST_p", C_S_ST_p)  # cut off for vizualisation
        ana.addData("C_S_Mesophyll_p", C_S_Mesophyll_p)  # cut off for vizualisation
        ...
        if True:
            ana.write("results"+directoryN+"plot_"+str(int(simStartSim))+str(condition)+"at"+ str(ö) +".vtp", 
                      ...
                      "psi_p_symplasm",
                      "C_S_ST_i_p","C_S_Mesophyll_i_p",
                      "C_S_ST_p","C_S_Mesophyll_p"]) 
    ...
    #     fluxes_p = np.insert(fluxes_p,0,0)# "[sucrose]",
        ...
        if((simDuration > 0) and (dtVTP >=0.29/24)):
            ...
            vp.plot_plant(r.plant,p_name = [ "fluxes","Exud","Gr","Rm","xylem pressure (cm)","sucrose concentration (mmol/cm3)",
                                                "C_S_ST_p","C_S_Mesophyll_p"],
                                vals =[ fluxes_p, Q_Exud_i_p, Q_Gr_i_p, Q_Rm_i_p, psiXyl_p, C_ST_p, C_S_ST_p, C_S_Mesophyll_p], 
        ...
        if dtPrint >= 0.9/24:
            ...
            write_file_array("Q_S_ST", Q_S_ST)
            write_file_array("Q_S_Mesophyll", Q_S_Mesophyll)
            ...
            if (min(r.Ev) < 0) or (min(r.Jw) < 0) or (min(fluxes_leaves)<0):
                print("leaf looses water", min(r.Ev),min(r.Jw), min(fluxes_leaves),r.loop)
                raise Exception
        ...
        Q_S_STbu       =   np.concatenate((Q_S_ST, np.full(Nt - Ntbu, 0.)))
        Q_S_mesobu     =   np.concatenate((Q_S_Mesophyll, np.full(Nt - Ntbu, 0.)))
...
if __name__ == '__main__':
    ...
    #simEnd = str(28)
    directoryN = "/"+os.path.basename(__file__)[:-3]+"/"+simInit+"to"+spellStart+"to"+spellEnd+"to"+simEnd+condition+"testConv/"
    print(directoryN)
