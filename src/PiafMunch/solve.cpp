...
void PhloemFlux::update_viscosity() { // called if (Adv_BioPhysics)
   ...
   for (int i=1 ; i <= Nc ; i++) { // this loop computes the viscosity profile, temporarily stored as vector r_ST_ before scaling to actual r_ST values:
		...
		//std::cout<<i<<") "<<C <<" "<<d<<" "<<siEnne<<" "<<mu <<" "<<r_ST_ref[i]<<" "<<r_ST[i]<<" ";
   }
}
...
void PhloemFlux::f(double t, double *y, double *y_dot) { // the function to be processed by the solver
    ...
    // vol_ST.display();
	//int neqsolve = 
	//cout<<endl<<"C_ST1 "<<Nt<<" "<<Nc<<" "<<R_<<" "<<T<<" "<<RT<<endl;
	...
	for (int i=1; i<=Nt; i++)  {
		...
		//if (Q_Sympl[i] < 0.){ QSTi = 0. ; Q_ST[i] =0;}// fix any artefact from solver (may try C<0 even if actual C never does)
		//assert(QSTi >= 0.);
		C_ST[i] = QSTi / volSTi ; // Concentration of sugar in sieve tubes		(mmol / ml)
		//cout<<", "<<i<<") "<< C_ST[i] <<" "<< QSTi <<" "<< volSTi<<" "<<(Q_Sympl[i]/volSTi);
		//std::cout<<" "<<(Starch[i]/volSTi)<<" "<<(Q_PhlApo[i]/volSTi)<<std::endl;
	...
	// C_ST.display();
	// double PartMolalVol_ = 0.2155;
	// dummy.set(1.) ; dummy.sub_mult(PartMolalVol_, C_ST) ; // dummy = 1 - PartMolalVol * C_ST
	// P_ST.set_elediv(C_ST, dummy) ; //  m_  =  C_ST / (1 - PartMolalVol * C_ST) ;
	// P_ST *= _0089_099803 * P_ST + 0.998 ; // = m_ * (0.998 + 0.089/0.99803 * m_) =  M_ST ; true Psi_ST value will be set later
	// P_ST *=  (RT*dEauPure) ;//dEauPure in mg ml-3
	
	
	//JW_PhlMb =  (Psi_ST - Psi_PhlApo)/r_PhlMb
	//(7) Psi_ST = Psi_PhlApo - r_PhlMb * JW_PhlMb  ;									// Nt scalar equations
	...
	for(int j = 1 ; j <= Nc ; j ++) {
		...
		if(JW_ST[j] > 0)
		{  
			i_amont[j] = I_Upflow[j] ; //i_aval= I_Downflow[j];
		}else{  
			i_amont[j] = I_Downflow[j] ; //i_aval= I_Upflow[j];
		}
		C_amont[j] = C_ST[i_amont[j]] ; //C_aval  = C_ST[i_aval] ;
		...
		//if (C_aval < 0.) C_aval = 0. ; // fix any artefact from solver (may try C<0 even if actual C never does)
		// if(JW_ST[j] != 0){
            // std::cout<<R<<" "<<T<<" "<<t<<std::endl;
            // std::cout<<j<<" "<<I_Upflow[j]<<" "<<I_Downflow[j]<<std::endl;
            // std::cout<<C_ST[I_Upflow[j]]<<" "<<C_ST[I_Downflow[j]]<<std::endl;
            // std::cout<<C_ST[I_Upflow[j]]*RT<<" "<<C_ST[I_Downflow[j]]*RT<<std::endl;
            // std::cout<<P_ST[I_Upflow[j]]<<" "<<P_ST[I_Downflow[j]]<<std::endl;
            // std::cout<<r_ST[i]<<" "<<JW_ST[j]<<std::endl;
            // assert(false);
		// }
		//check water equilibrium assumption:
		//RhatFhat[j] = 2/a_ST[i_aval] * RT * C_aval * length[i_aval] /  JW_ST[j] * Lp;
	}
	...
	if(hayErrores){
		...
		JS_ST.display();
		for(int j = 1 ; j <= Nc ; j ++) {
			std::cout<<j<<") "<<JW_ST[j]<<" "<< C_ST[I_Upflow[j]]<<" "<< C_ST[I_Downflow[j]]<<" "<<C_amont[j]<<", ";
		}
		assert(false);
