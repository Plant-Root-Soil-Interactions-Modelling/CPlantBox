...
//
//    !!!     Le symbole '*' représente la multiplication ELEMENTWISE, COLONNE PAR COLONNE     !!!
//    La multiplication matricielle est notée  matmult( ), le produit scalaire est noté  dotprod( ),
//    et la multiplication elementwise ligne par ligne (in-place ou non)  est notée  row_elemult( )
//
// ----------------------------------------------------------------------------------------------------------------------------
...
/******** Arithmétique composite 'in-place' commune à (presque) toutes les classes, en plus de celles propres à chacune *****/
...
class Fortran_matrix
{
  private:
	...
	double* v_;						// 'vecteur'[1 + m_*n_] contenant, rangés par colonnes successives, ...
									// ...les éléments de la matrice dont le nombre est stocké en v_[0] = m_*n_
  public:
	Fortran_matrix(int m = 0);							// matrice carrée d'ordre m, NON INITIALISEE
	Fortran_matrix(int m, int n);						// matrice à m lignes x n colonnes, NON INITIALISEE
	Fortran_matrix(int m, int n, const double &a);		// matrice à m lignes x n colonnes, initialisée à 'a'
	Fortran_matrix(const Sparse_matrix &S);				// crée une matrice pleine à partir d'une creuse existante
	Fortran_matrix(const SpUnit_matrix &U);				// crée une matrice pleine à partir d'une creuse à +- 1.
	Fortran_matrix & operator=(const Fortran_matrix &A);// opérateur de recopie
	~Fortran_matrix();
	...
	inline const double& operator()(int i, int j) const { // retourne la valeur de l'élément d'indices i,j
		if ((i < 1) || (i > m_) || (j < 1) || (j > n_)) assert(false) ;
		return v_[ i + (j-1)*m_];
	}
	inline double & operator()(int i, int j) {			// réaffecte l'élément d'indices (i,j)
		if ((i < 1) || (i > m_) || (j < 1) || (j > n_)) assert(false) ;
		return v_[ i + (j-1)*m_];
	}
	/**** Fonctions externes déclarées 'friend' de la classe Fortran_matrix ***********************************/
	friend class Fortran_vector;							// imposé par un croisement entre les 2 classes / arithmétique composite
...
{
  private:
	double* v_;						// 'vecteur'[n+1]: n éléments stockés en v[1]..v[n] , et v[0] = n ;

  public:
	Fortran_vector(int size = 0);							// 'size' éléments, non initialisés
	Fortran_vector(vector<double> vecIn, double factor = 1.);// transform from cppvec to fortran vec
	Fortran_vector(int size, const double &a);				// 'size' éléments, initialisés à la valeur a
	Fortran_vector(int size, const double *b, bool ignore_zeroindex = true); // v.v_ = recopie de b (donc indépendant)
    Fortran_vector(const Fortran_vector &v);				// crée une copie du vecteur v
	Fortran_vector & operator=(const Fortran_vector &v);	// opérateur de recopie
	~Fortran_vector();
	...
	void set(const double &a);								// affecte la valeur indiquée à tous les éléments
	void set(const Fortran_vector &v);						// vecteur argument = v, qui doit être de même taille
	void append(const Fortran_vector & v) ;					// fusion avec v en queue, eq. à  X = bind(X, v)
	void sequentialFill(std::vector<double> vecd, int smallVal, int bigVal);
	void zero(const Fortran_vector &z) ; // annule toute composante [i] inférieure en val.abs. à z[i]

	inline const double& operator()(int i) const {			// retourne la valeur de l'élément d'indice i
		if ((i < 1) || (i > (int)v_[0])) assert(false) ;
		return v_[i];
	}
	inline double & operator()(int i) {						// réaffecte l'élément d'indice i
		if ((i < 1) || (i > (int)v_[0])) assert(false) ;
		return v_[i];
	}
	inline const double& operator[](int i) const {			// retourne la valeur de l'élément d'indice i
	...
	Fortran_vector operator-() ;							// opérateur unaire de changement de signe
	...
	/******************************* Arithmétique vectorielle 'inplace' composite  ******************************************/
	//  Dans la série 'void add_XXX()', le dernier argument 'ad = 1', optionnel et invisible à l'utilisateur final, oriente vers
	// 'set_xxx()' ou 'sub_xxx()', soit par une macro #define (cf. en-tête de ce fichier), soit par une fonction vraie (X_matmult())
	void add_subvector(const int &i1, const int &i2, const Fortran_vector &v1, int ad = 1); // v[i1, i2] += v1
	void add_subvectorx(const Index_vector &index, const Fortran_vector &v, int ad = 1); //...spécifiés par 'index'
	...
	int set_KLU_solve(const Sparse_matrix &S, const Fortran_vector &y, int** ipiv_ptr = NULL, void** TM_ptr = NULL, bool full_check = false);// résoud   S*x = y    en retournant  x =  inv(S) * y

	/**** Fonctions externes déclarées 'friend' de la classe Fortran_vector ***********************************/
    ...
    inline double* InPlace_Array(Fortran_vector &v) {return v.v_;} // attention, fonction dangereuse à utiliser avec précaution !!
...
{
  private:
	int* v_; // 'vecteur'[n+1]: n éléments (indices) stockés en v[1]..v[n] , et v[0] = n ;

  public:
	Index_vector(int size = 0); // crée un tableau de 'size' valeurs d'indices, initialisées à 0
	Index_vector(int size, const int start, const int incr = 1); // séquence (start=début, incr=incrément)
    Index_vector(const Index_vector &iv);	// crée une copie d'un index_vector existant
	Index_vector & operator=(const Index_vector &iv); // opérateur de recopie
	int & operator()(int i);				// affectation d'un élément du tableau d'indices
	const int & operator()(int i) const ; // retourne un élément du tableau d'indices
	int & operator[](int i);				// affectation d'un élément du tableau d'indices
	const int & operator[](int i) const ; // retourne un élément du tableau d'indices
	int size() const;				// nombre d'éléments (valeurs d'indices) du vecteur considéré
	void append(const Index_vector & iv) ; // ajoute iv.size() valeurs d'indices supplémentaires
	~Index_vector();
...
{
  private:
	int m_, n_, ntnz_ ,npnz_;  // resp.: nb de lignes, de colonnes et d'éléments POTENTIELLEMENT non-nuls de la matrice , et d'éléments REELLEMENT non-nuls ('number of true NZ').
	int * ij_ ;			// array à (n_ + m_ + 2 * npnz_) éléments, successivement :
						//		ni_[j = 1..n_] et nj_[i = 1..m_] = nb d'éléments réellement non nuls, resp. de la colonne #j et de la ligne #i  -- sigma(ni_[j-1]) = sigma(nj_[i-1]) = ntnz_
						//		cmi_[1..ntnz_]  = indices i des ntnz_ éléments réellement non-nuls (col-major),   SUIVI DE (npnz-ntnz) ZÉROS  =>  LONGUEUR TOTALE DU SUB-ARRAY cmi_ = npnz
						//		rmj_[1..ntnz_]  = indices j des ntnz_ éléments réellement non-nuls (row-major),   SUIVI DE (npnz-ntnz) ZÉROS  =>  LONGUEUR TOTALE DU SUB-ARRAY rmj_ = npnz
	double * v_ ;		// (2 * npnz élts) : cmv_[1..ntnz_] suivi de (npnz-ntnz) zéros, puis rmv_[1..ntnz_] suivi de (npnz-ntnz) zéros = valeurs des npnz elts 'non nuls', resp. col_maj. et row_maj.

  public:
	Sparse_matrix() ;							// constructeur par défaut (matrice nulle, 0 x 0)
	Sparse_matrix(const int &m, const int &n, const int &npnz = 0) ; // initialize les dimensions, y compris npnz_
	Sparse_matrix(const Sparse_matrix &S, const int &npnz = -1) ;		// constructeur par recopie ; si npnz = -1 (défaut), garde le S.npnz_ ; si = -2, restreint npnz_ à ntnz_ .
	Sparse_matrix(const SpUnit_matrix &U) ;		// remplace 'true' par 1. et 'false' par -1.
	Sparse_matrix & operator=(const Sparse_matrix &S) ;// opérateur de recopie : garde le S.npnz_
	~Sparse_matrix();
	int nblin() const;									// retourne le nombre de lignes de la matrice
	int nbcol() const;									// retourne le nombre de colonnes de la matrice
	int ntnz() const ;									// retourne le nombre total d'éléménts non nuls
	int npnz() const ;									// retourne le nombre maximal d'éléménts non nuls
	void reset_npnz(int npnz = 0) ;	// si npnz <= 0, npnz_ = ntnz_ ; sinon, npnz_ doit être >= ntnz_.
	void display(int i1 = 1, int i2 = 0, int j1 = 1, int j2 = 0) // display M[i1:i2, j1:j2] (default: i2 = 0 -> = m_ ; j2 = 0 -> = n_
		{ (Fortran_matrix(*this)).display(i1, i2, j1, j2) ; }
	void dump() ; // affiche tous les attributs et les 4 arrays.
	void set(const Sparse_matrix &S, bool check = true) ;	// S_ = S ; les 2 matrices doivent être de mêmes dimensions et ntnz.  (n'impose pas l'égalité des npnz_) -- check : vérifie nb et position des nz
	const double& operator()(int i, int j) const ;  // retourne la valeur de l'élément d'indices i,j
	void add_(int i, int j, double a, int ad = 1) ;	// affecte (si ad =0 : macro 'set_'), ajoute (si (ad =1 : 'add_' s.s.) ou soustrait (si ad =-1 : macro 'sub_') la valeur a à l'élément d'indices (i,j)
	Sparse_matrix operator-() ;							// opérateur unaire de changement de signe
	...
	//  Dans la série 'void add_XXX()', le dernier argument 'ad = 1', optionnel et invisible à l'utilisateur final, oriente vers
	// 'set_xxx()' ou 'sub_xxx()', soit par une macro #define (cf. en-tête de ce fichier), soit par une fonction vraie (x_matmult())
	void add_add(const Sparse_matrix &S1, const Sparse_matrix &S2, int ad = 1, bool save_count = true) ; // seul (ad = 0) est accepté
	void add_add(const Sparse_matrix &S, const SpUnit_matrix &U, int ad = 1, bool save_count = true) ; // seul (ad = 0) est accepté
	void add_add(const SpUnit_matrix &U, const Sparse_matrix &S, int ad = 1, bool save_count = true) ; // seul (ad = 0) est accepté
	void add_add(const SpUnit_matrix &U1, const SpUnit_matrix &U2, int ad = 1, bool save_count = true) ; // seul (ad = 0) est accepté
	void add_sub(const Sparse_matrix &S1, const Sparse_matrix &S2, int ad = 1, bool save_count = true) ; // seul (ad = 0) est accepté
	void add_sub(const Sparse_matrix &S, const SpUnit_matrix &U, int ad = 1, bool save_count = true) ; // seul (ad = 0) est accepté
	void add_sub(const SpUnit_matrix &U, const Sparse_matrix &S, int ad = 1, bool save_count = true) ; // seul (ad = 0) est accepté
	void add_sub(const SpUnit_matrix &U1, const SpUnit_matrix &U2, int ad = 1, bool save_count = true) ; // seul (ad = 0) est accepté
	void add_elemult(const Sparse_matrix &S1, const Sparse_matrix &S2, int ad = 1, bool save_count = true); // seul (ad = 0) est accepté
	void add_elemult(const Sparse_matrix &S, const SpUnit_matrix &U, int ad = 1, bool save_count = true); // seul (ad = 0) est accepté
	void add_elemult(const SpUnit_matrix &U, const Sparse_matrix &S, int ad = 1, bool save_count = true); // seul (ad = 0) est accepté
	...
	friend class Fortran_vector;
	friend class SpUnit_matrix;
	friend Sparse_matrix transpose(const Sparse_matrix & S, int npnz); // si npnz = -1 (défaut), garde le S.npnz_ ; si = -2, restreint npnz_ à ntnz_ .
	...
	friend void KLU_allocate(const Sparse_matrix &S, int*** ipiv_ptr_ptr, void*** TM_ptr_ptr); // crée les espaces de travail appropriés pour la matrice S
	...
	Sparse_matrix diag(const double &a, const int m, int n = -1);		// diag.: a * Id(m,n); carrée (m,m) si n = -1
	Sparse_matrix diag(const Fortran_vector & V);			// construit la matrice carrée diagonale <- vect. V
	Sparse_matrix diag(const Fortran_vector & V, const int m, const int n);//id, rectang.; V.size() = min(m,n)
	Sparse_matrix transpose(const Sparse_matrix & S, int npnz = -1); // si npnz = -1 (défaut), restreint npnz_ à ntnz_ ; si = -2, garde le S.npnz_.
	...
	Fortran_vector KLU_solve(const Sparse_matrix &S, const Fortran_vector &y, int** ipiv_ptr = NULL, void** TM_ptr = NULL, bool full_check = false); // résoud   S*x = y    en retournant  x =  inv(S) * y
...
{
  private:
	int m_, n_, nnz_ ;  // resp.: nb de lignes, de colonnes et d'éléments non-nuls de la matrice
	int * ij_ ;			// array à (m_ + n_ + 4*nnz_) éléments, successivement :
						//		ni_[j = 1..n_] et nj_[i = 1..m_] = nb d'éléments non nuls, resp. de la colonne #j et de la ligne #i,
						//		cmi_[1..nnz_] et rmj_[1..nnz_] = indices des éléments non-nuls, resp. i (col-major) et j (row-maj.);
						//		cmv_[1..nnz_] et rmv[1..nnz_] = valeurs des nnz elts, resp. col_maj. et row_maj, codés: +1 = true, -1 = false
  public:
	SpUnit_matrix() ;							// constructeur par défaut (matrice nulle, 0 x 0)
	...
	SpUnit_matrix & operator=(const SpUnit_matrix &U);// opérateur de recopie
	~SpUnit_matrix();
	int nblin() const;									// retourne le nombre de lignes de la matrice
	int nbcol() const;									// retourne le nombre de colonnes de la matrice
	int nbnz() const ;									// retourne le nombre total d'éléménts marqués comme non nuls
	void display(int i1 = 1, int i2 = 0, int j1 = 1, int j2 = 0)// display M[i1:i2, j1:j2] (default: i2 = 0 -> = m_ ; j2 = 0 -> = n_
		{ (Fortran_matrix(*this)).display(i1, i2, j1, j2) ; }

	void set(const SpUnit_matrix &U);					// U_ = U ; LES NZ (vrais) DOIVENT ETRE SYNCHRO dans les 2 matrices
	SpUnit_matrix operator-() ;							// opérateur unaire de changement de signe
	Sparse_matrix operator+(const Sparse_matrix &S) ;
	Sparse_matrix operator+(const SpUnit_matrix &U) ;
	Sparse_matrix operator-(const Sparse_matrix &S) ;
	Sparse_matrix operator-(const SpUnit_matrix &U) ;
	...
	void add_elemult(const SpUnit_matrix &U1, const SpUnit_matrix &U2, int ad = 1, bool save_count = true); // seul (ad = 0) est accepté
