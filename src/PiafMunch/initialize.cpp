...
void PhloemFlux::initialize_carbon(vector<double> vecIn) {
	...
	std::cout<<"vecIn.size() ";std::cout<<vecIn.size()<<" "<<Nt<<" "<<Nc<<" "<<std::endl;
    if(vecIn.size() == (Nt*neq_coef)){
			std::cout<<"setup full y0 "<<std::endl;
		...
		if(vecIn.size() > 0){
			...
			// Q_PhlApo.display();
			// Q_ParApo.display();
			//Y0= Fortran_vector(0.;Nt*8);
			//int Nt_old = int(vecIn.size())/8;
			//
			...
			std::cout<<"setup leer y0 "<<std::endl;
			Y0 =  Fortran_vector(Nt*neq_coef, 0.) ;
			...
			if(withInitVal){
				...
				this->Q_init = Y0.toCppVector();
			}
		}
	}
	
	//Y0 =  Fortran_vector(Nt*5, 0.) ;
	
	cout<<"Y0_STinit "<<Y0[1]<<" "<<Nc<<" "<<Nt<<" "<<Nt_old<<endl;
	Nt_old = Nt; //BU Nt
	//C_ST.display();
	//Y0.display();
...
void PhloemFlux::initialize_hydric() {
	...
	//Absorb = Fortran_vector(N1R, 0.)		; // Root end water absorbtion flux										(ml / h)
	...
	// The following 2 environment data are actually set in function   hydric_boundary_conditions()  at the end of file 'water-fluxes.cpp' :
	//Transpirat = Fortran_vector(N1L, 0.) ; // (mmol / h) Leaf transpiration rate (used in water-fluxes calc.) -- may be specified in real-time  in  water-fluxes.cpp::hydric_boundary_conditions()
	//PsiSoil = Fortran_vector(N1R, 0.)		; // (MPa) Soil water potential at root end (used in water-fluxes calc.) -- may be specified in real-time  in  water-fluxes.cpp::hydric_boundary_conditions()
	//if (Adv_BioPhysics) PartMolalVol = 0.2155 ; else PartMolalVol = 0. ; // (L / mol) valeur Thompson et Holbrook ; en fait serait plutôt 0.214, indépendant (à 0.1% près) à la fois de T et de C (cf. SucroseViscosity.xls)
	...
	for(j = 1 ; j <= Nc ; j ++) { 
		...
		//cout<<endl<<I_Upflow[j]<<" "<<I_Downflow[j]<<" "<<j<<endl;
		...
		//cout<<endl<<"nodes part "<<endl;
		//cout<<endl<<"Nt, Nc "<<Nt<<" "<<Nc<<endl;
	...
	//Delta.display();
		//cout<<endl<<"Nt, Nc "<<Nt<<" "<<Nc<<endl;
	// sizing permanent auxiliary matrices and vectors that will speed up solving :
	//dummy = Fortran_vector(Nt) ; dummy_= Fortran_vector(Nc) ; // temporary buffers
	Delta_rphl = Delta / r_ST_ref ;
			
	ipiv_ptr = NULL ; TM_ptr = NULL;
	//KLU_allocate(X, &ipiv_ptr, &TM_ptr);
	//Psi_Xyl.display();
...
//	cout << "at t=" << t << ": resistance_changed = " << resistance_changed << endl ;
