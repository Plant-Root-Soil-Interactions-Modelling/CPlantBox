...
class PlantHydraulicModel(PlantHydraulicModelCPP):
    ...
    def sinusoidal2(t, dt):
        ...
        return np.maximum(0.0, np.pi * (np.cos(2 * np.pi * (t - 0.5)) + np.cos(2 * np.pi * ((t + dt) - 0.5))) / 2)
    ...
    def get_ages(self, final_age=-1.0):
        """ages per segment
        ...
        for seg_id, cell_id in map.items():
            if cell_id < 0:
                print(
                    "Warning: segment ",
                    seg_id,
                    "is not mapped, this will cause problems with coupling!",
                    "organType",
                    ots[seg_id],
                    "subType",
                    types[seg_id],
    ...
    def test(self):
        ...
        for seg_id, cell_id in map.items():
            ...
            if (cell_id < 0) & (types[seg_id] == int(pb.root)):
                print(
                    "Warning: root segment ",
                    seg_id,
                    "is not mapped (out of the soil domain), this could cause problems with coupling!",
                    nodes[segments[seg_id][0]],
                    nodes[segments[seg_id][1]],
                )
        if len(nodes) > len(segments) + 1:
            print("Warning: there are more nodes than segments, some nodes are not connected to the root system")
            print(nodes[len(segments) + 1 :])
