...
def init_conductivities(r, age_dependent: bool = False):
    ...
    if isinstance(r, PlantHydraulicParameters):
        if age_dependent:
            r.set_kr_age_dependent(kr0[:, 0], kr0[:, 1], subType=[0, 1])
            r.set_kr_age_dependent(kr1[:, 0], kr1[:, 1], subType=[2, 3, 4, 5])
            r.set_kx_age_dependent(kx0[:, 0], kx0[:, 1], subType=[0, 1])
            r.set_kx_age_dependent(kx1[:, 0], kx1[:, 1], subType=[2, 3, 4, 5])
        else:  # we set it as table to be able to make the rootsystem grow in a predefined way
            r.set_kr_const(kr_const_)
            r.set_kx_const(kx_const_)
    else:

        if age_dependent:
            r.setKrTables([kr0[:, 1], kr0[:, 1], kr1[:, 1], kr1[:, 1], kr1[:, 1], kr1[:, 1]], [kr0[:, 0], kr0[:, 0], kr1[:, 0], kr1[:, 0], kr1[:, 0], kr1[:, 0]])  # [cm^3/day]
            r.setKxTables([kx0[:, 1], kx0[:, 1], kx1[:, 1], kx1[:, 1], kx1[:, 1], kx1[:, 1]], [kx0[:, 0], kx0[:, 0], kx1[:, 0], kx1[:, 0], kx1[:, 0], kx1[:, 0]])  # [1/day]

        else:  # we set it as table to be able to make the rootsystem grow in a predefined way
            kr = np.array([[-1e4, 1.0e-9], [-1.0e-9, 1.0e-9], [0.0, kr_const_], [1e4, kr_const_]])
            kx = np.array([[0, kx_const_], [1e4, kx_const_]])
            r.setKrTables([kr[:, 1], kr[:, 1], kr[:, 1], kr[:, 1], kr[:, 1], kr[:, 1]], [kr[:, 0], kr[:, 0], kr[:, 0], kr[:, 0], kr[:, 0], kr[:, 0]])  # [cm^3/day]
            r.setKxTables([kx[:, 1], kx[:, 1], kx[:, 1], kx[:, 1], kx[:, 1], kx[:, 1]], [kx[:, 0], kx[:, 0], kx[:, 0], kx[:, 0], kx[:, 0], kx[:, 0]])  # [1/day]
...
def init_conductivities_growth(r, age_dependent: bool = False, dt=1):
    ...
    if age_dependent:
        r.setKrTables([kr0[:, 1], kr1[:, 1], kr1[:, 1], kr1[:, 1], kr1[:, 1], kr1[:, 1]], [kr0[:, 0], kr1[:, 0], kr1[:, 0], kr1[:, 0], kr1[:, 0], kr1[:, 0]])  # [cm^3/day]
        r.setKxTables([kx0[:, 1], kx1[:, 1], kx1[:, 1], kx1[:, 1], kx1[:, 1], kx1[:, 1]], [kx0[:, 0], kx1[:, 0], kx1[:, 0], kx1[:, 0], kx1[:, 0], kx1[:, 0]])  # [1/day]

    else:  # we set it as table to be able to make the rootsystem grow in a predefined way
        kr = np.array([[-1e4, 1.0e-9], [0.0, 1.0e-9], [dt, kr_const_], [1e4, kr_const_]])
        kx = np.array([[0, kx_const_], [1e4, kx_const_]])
        r.setKrTables([kr[:, 1], kr[:, 1], kr[:, 1], kr[:, 1], kr[:, 1], kr[:, 1]], [kr[:, 0], kr[:, 0], kr[:, 0], kr[:, 0], kr[:, 0], kr[:, 0]])  # [cm^3/day]
        r.setKxTables([kx[:, 1], kx[:, 1], kx[:, 1], kx[:, 1], kx[:, 1], kx[:, 1]], [kx[:, 0], kx[:, 0], kx[:, 0], kx[:, 0], kx[:, 0], kx[:, 0]])  # [1/day]
...
def init_conductivities_scenario_jan(r):
    ...
    kr0 = np.array([[0.0, 0.000181], [8.0, 0.000181], [10, 0.0000648], [18, 0.0000648], [25, 0.0000173], [300, 0.0000173]])
    kr1 = np.array([[0.0, 0.000181], [10.0, 0.000181], [16, 0.0000173], [300, 0.0000173]])
    kr2 = np.array([[0.0, 0.000181], [10.0, 0.000181], [16, 0.0000173], [300, 0.0000173]])

    # axial values # [1/day]
    # (ageKh(1,i),KhRoot(1,i),i=1,nKh(1..3))
    kx0 = np.array([[0.0, 0.0000864], [5.0, 0.00173], [12.0, 0.0295], [15.0, 0.0295], [20.0, 0.432], [300.0, 0.432]])
    kx1 = np.array([[0.0, 0.0000864], [5.0, 0.0000864], [10.0, 0.0000864], [12.0, 0.0006048], [20.0, 0.0006048], [23.0, 0.00173], [300.0, 0.00173]])
    kx2 = np.array([[0.0, 0.0000864], [5.0, 0.0000864], [10.0, 0.0000864], [12.0, 0.0006048], [20.0, 0.0006048], [23.0, 0.00173], [300.0, 0.00173]])
...
def init_conductivities_scenario_jan_const(r):
    ...
    kr = np.array([[0.0, kr_const], [1e4, kr_const]])
    r.setKrTables([kr[:, 1], kr[:, 1], kr[:, 1], kr[:, 1], kr[:, 1], kr[:, 1]], [kr[:, 0], kr[:, 0], kr[:, 0], kr[:, 0], kr[:, 0], kr[:, 0]])  # [cm^3/day]
    # first entry twice, because rsml starts at type 1

    kx_const = 2.95e-2  # [cm3/day]
    kx = np.array([[0.0, kx_const], [1e4, kx_const]])

    r.setKxTables([kx[:, 1], kx[:, 1], kx[:, 1], kx[:, 1], kx[:, 1], kx[:, 1]], [kx[:, 0], kx[:, 0], kx[:, 0], kx[:, 0], kx[:, 0], kx[:, 0]])  # values, age
