...
class PlantHydraulicParameters(PlantHydraulicParametersCPP):
    ...
    def kx_f(self, age, st, ot = pb.OrganTypes.root, seg_ind = 0):
    ...
    def plot_conductivities(self, monocot = True, plot_now = True, axes_ind = [], lateral_ind = []):
        ...
        for j, st in enumerate(axes_ind):
            kx_ = [ self.kx_f(axes_age[i], int(st)) for i in range(0, len(axes_age)) ]
        ...
        for j, st in enumerate(lateral_ind):
            kx_ = [ self.kx_f(lateral_age[i], st) for i in range(0, len(lateral_age)) ]
        ...
        for j, st in enumerate(axes_ind):
            kr_ = [ self.kr_f(axes_age[i], st) for i in range(0, len(axes_age)) ]
        ...
        for j, st in enumerate(lateral_ind):
            kr_ = [ self.kr_f(lateral_age[i], st) for i in range(0, len(lateral_age)) ]
        ...
        print()
        for st in range(0, 5):
            print("SubType {:g} for negative age: kx = {:g}, kr = {:g}".format(st, self.kx_f(-1, st), self.kr_f(-1, st)))
            print("SubType {:g} for old root age: kx = {:g}, kr = {:g}".format(st, self.kx_f(100, st), self.kr_f(100, st)))
        print()
        if plot_now:
            plt.tight_layout()
            plt.show()
