...
from plantbox import PhloemFlux  # defined in PiafMunch/runPM.h
...
class PhloemFluxPython(PhloemFlux, PhotosynthesisPython):
    ...
    def solve_phloem_flow(self, dt, simDuration=None, TairC=None, verbose=False, outputfile="errors_solve_phloem_flow.txt"):
        ...
        self.Q_STbu = np.concatenate((self.Q_STbu, np.full(self.Nt - self.Ntbu, 0.0)))
        self.Q_Rmbu = np.concatenate((self.Q_Rmbu, np.full(self.Nt - self.Ntbu, 0.0)))
        self.Q_Grbu = np.concatenate((self.Q_Grbu, np.full(self.Nt - self.Ntbu, 0.0)))
        self.Q_Exudbu = np.concatenate((self.Q_Exudbu, np.full(self.Nt - self.Ntbu, 0.0)))
    ...
    def getPsiAir(self, RH, TairC):  # constants are within photosynthesys.h
        return np.log(RH) * self.rho_h2o * self.R_ph * (TairC + 237.3) / self.Mh2o * (1 / 0.9806806)
        # in cm
    ...
    def get_ages(self, final_age=0.0):
    ...
    def get_suf(self, sim_time):
        ...
        rx = self.solve_neumann(sim_time, -1.0e5, p_s, False)  # False: matric potential not given per cell (but per segment), high number to recuce spurious fluxes
        ...
        return np.array(fluxes) / -1.0e5  # [1]
    ...
    def get_krs(self, sim_time, seg_ind=[0]):
        ...
        for i, s in enumerate(segs):
            p_s[i] = -500 - 0.5 * (nodes[s.x].z + nodes[s.y].z)  # constant total potential (hydraulic equilibrium)
        rx = self.solve_dirichlet(sim_time, -15000, 0.0, p_s, cells=False)
        jc = 0
        for i in seg_ind:
    ...
    def write_phloem_parameters(self, filename="phloem_parameters"):
        ...
        parameters = {
            ...
            "Soil": {"DefaultC": {"value": self.CsoilDefault, "unit": "mmol Suc cm-3", "description": "dummy value for soil concentration"}},
            "Solver": {
