 ...
 */
Leaf::Leaf(int id, const OrganSpecificParameter* param, bool alive, bool active, double age, double length,
 ...
 */
Leaf::Leaf(Organism* plant, int type, Vector3d iheading, double delay, Organ* parent, double pbl, int pni) :Organ(plant,parent,Organism::ot_leaf, type,delay), parentBaseLength(pbl), parentNI(pni)
{
    iHeading=iheading;
...
{
    oldNumberOfNodes = 0; // is set in Leaf:createSegments, the zero indicates the first call to createSegments
    ...
    if (alive) { // dead leaf wont grow
        ...
        if (age>0) {
            ...
            if (active) {
                ...
                // length increment
                double length_ = calcLength(std::max(age-dt,0.)); // length of the leaf for unimpeded f_gf (i.e. length_==length for unimpeded f_gf)
            ...
            active = calcLength(std::max(age,0.))<(param()->getK()-dx()/10); // become inactive, if final length is nearly reached
        }
    } // if alive
...
double Leaf::calcCreationTime(double length)
{
    ...
    if (parent->organType()!=Organism::ot_stem) {
        if (parent->organType()==Organism::ot_leaf) {
            double pl = parentBaseLength+((Leaf*)parent)->getLeafRandomParameter()->la; // parent length, when this leaf was created
            double pAge=((Leaf*)parent)->calcCreationTime(pl);
            return age+pAge;
...
void Leaf::createSegments(double l, bool silence)
{
    ...
    int nn = nodes.size();
    if (oldNumberOfNodes==0) { // first call of createSegments (in leaf::simulate)
        if (nn>1) {
