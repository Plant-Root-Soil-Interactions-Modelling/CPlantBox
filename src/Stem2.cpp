 ...
 */
Stem::Stem(int id, const OrganSpecificParameter* param, bool alive, bool active, double age, double length,
 ...
 */
Stem::Stem(Organism* rs, int type, Vector3d heading, double delay,  Organ* parent, double pbl, int pni) :Organ(rs, parent, Organism::ot_root, type, delay)
{
    iHeading = heading;
	StemRandomParameter* sttp = getStemRandomParameter();

//        std::cout <<"subtype ="<<stem_p->subType <<"stem getPhytomerId =" <<getphytomerId(stem_p->subType)<< "\n";
	addPhytomerId(param()->subType);
	double beta = getphytomerId(param()->subType)*M_PI*sttp->RotBeta + M_PI*plant->rand()*sttp->BetaDev ;  //+ ; //2 * M_PI*plant->rand(); // initial rotation
	Matrix3d ons = Matrix3d::ons(iHeading);
	if (sttp->InitBeta >0 && getphytomerId(param()->subType)==0 ){
 ...
 */
Organ* Stem::copy(Organism* rs)
{
    Stem* r = new Stem(*this); // shallow copy
    r->parent = nullptr;
    r->plant = rs;
    r->param_ = new StemSpecificParameter(*param()); // copy parameters
...
void Stem::simulate(double dt, bool verbose)
{
    ...
    if (alive) { // dead roots wont grow
        ...
        if ((age>0) && (age-dt<=0)) { // the root emerges in this time step
            double P = getStemRandomParameter()->f_sbp->getValue(nodes.back(),this);
        ...
        if (age>0) { // unborn  roots have no children
            ...
            if (active) {
                ...
                double scale = getStemRandomParameter()->f_sa->getValue(nodes.back(),this);
...
double Stem::calcLength(double age)
{
    ...
    return getStemRandomParameter()->f_gf->getLength(age,param()->r,param()->getK(),this);
}
...
double Stem::calcAge(double length)
{
    ...
    return getStemRandomParameter()->f_gf->getAge(length,param()->r,param()->getK(),this);
}
 ...
 */
const StemSpecificParameter* Stem::param() const
{
    return (const StemSpecificParameter*)param_;
}
...
{
    int lt = getStemRandomParameter()->getLateralType(nodes.back());
    if (lt>0) {
        ...
        Stem* lateral = new Stem(plant, lt,  heading(), delay,  this, length, nodes.size()-1);
 ...
 *  @param verbose  turns console output on or off
 */
void Stem::createSegments(double l, double dt, bool verbose)
{
    if (l==0) {
    ...
    if (firstCall) { // first call of createSegments (in Root::simulate)
        firstCall = false;
        if ((nn>1) && (children.empty() || (nn-1 != ((Stem*)children.back())->parentNI)) ) { // don't move a child base node
