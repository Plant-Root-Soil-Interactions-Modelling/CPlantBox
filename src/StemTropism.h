#ifndef STEMTROPISM_H
#define STEMTROPISM_H
...
public:
  ...
  */
  StemTropismFunction(double n_,double sigma_): n(n_), sigma(sigma_) { }; ///< Tropism with n_ number of trials and standard deviation of sigma_

  StemTropismFunction():StemTropismFunction(0,0) { } ///< Default constructor is TropismFunction(0,0)

  virtual Vector2d getHeading(const Vector3d& pos, Matrix3d old, double dx, const Organ* stem);
  ...
  */
  virtual double stemtropismObjective(const Vector3d& pos, Matrix3d old, double a, double b, double dx, const Organ* stem) { std::cout << "default\n"; return 0; }
...
*/
class ConfinedStemTropism : public StemTropismFunction
{
public:
  ...
  */
  ConfinedStemTropism(StemTropismFunction* baseTropism, SignedDistanceFunction* geometry) : StemTropismFunction(0,0), stemtropism(baseTropism), geometry(geometry) { }
  //< ConfinedTropism confines the baseTropism to geometry

  virtual Vector2d getHeading(const Vector3d& pos, Matrix3d old,  double dx, const Organ* stem) override;
...
private:
  StemTropismFunction* stemtropism;
...
*/
class StemGravitropism : public StemTropismFunction
{
...
class StemAntiGravitropism : public StemTropismFunction
{
...
*/
class StemPlagiotropism : public StemTropismFunction
{
...
*/
class StemExotropism : public StemTropismFunction
{

public:

  StemExotropism(double n, double sigma) : StemTropismFunction(n,sigma) { } ///< @see TropismFunction
 ...
 */
class CombinedStemTropism : public StemTropismFunction
{

public:

  CombinedStemTropism(double n, double sigma, std::vector<StemTropismFunction*> tropisms_, std::vector<double> weights_): StemTropismFunction(n,sigma), tropisms(tropisms_), weights(weights_) {
  ...
  CombinedStemTropism(double n, double sigma, StemTropismFunction* t1, double w1, StemTropismFunction* t2, double w2);
...
private:
  std::vector<StemTropismFunction*> tropisms;
