...
def getLengthsurfaces( plant ):
	...
	return np.concatenate(([lengths[0]/100],lengths)), np.concatenate(([surfacesPhloem[0]],surfacesPhloem)), np.concatenate(([surfacesOrg[0]],surfacesOrg)) #add surface and length for seed node
...
def write_PiafMunch_parameter(plant,initphi=[], name = "mogi_test", time=36, nopsi=False): #function of creating parameters of PiafMunch
	...
	for i, ot in enumerate(nodes_organtype): #given different value based on whether it is source, sink or connection
		...
		if i in N1R_node:   #root tip
			vMU[i]		= 0.9 #mol suc ml-1 s-1 => mmol C ml-1 hr-1
			
		if i in N1L_node : #stem
			vML[i]		= 1 #mol suc ml-1 s-1 => mmol C ml-1 hr-1
			


	volPhlApo = volParApo = vol_Sympl_max = vol_Sympl = vol_ST 
	...
	# np.full(len(nodes_organtype), 2.6e-005)
	...
	for i in range(len(nodes_organtype)):
		f.write("{:.0f}  {:e}  {:e} {:e} {:e} {:e}  {:e}  {:e} {:e} {:e} {:e} {:e}\n"
				.format(i+1, Q_ST[i],Q_Sympl[i],Starch,Q_PhlApo,Q_ParApo, 
						Tr_Q_ST,Tr_Q_Sympl,Tr_Starch,Tr_Q_PhlApo,Tr_Q_ParApo,vol_Sympl[i]))
	f.write('\n')	
