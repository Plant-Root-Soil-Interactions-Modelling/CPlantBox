...
class PhloemFluxPython(Leuning):
    ...
    def mp2mesh(self, segs): #create an old2newNodeID map?
        ...
        self.mesh = Mesh1Dmod(facesNorm=facesNorm, vertexCoords=vertices, faceVertexIDs=self.faces, cellFaceIDs=cells)
    ...
    def _setFreeOutflow(self): #or set dirichlet of 0?
        ...
        self.exteriorCoeff.constrain(value= self.osmoCoeff * self.phi.faceGrad, where=tiproots) #t change when flow defined in another way
        print('_setFreeOutflow::extcoef ',self.exteriorCoeff)
    ...
    def setAnSource(self): #sourceterm in leaf segment
        ...
        print('setAnSource::nodeYLeaf ', nodeYLeaf)        
        nodeYLeaf_newID = np.array([self.old2newNodeID[xi] for xi in nodeYLeaf]).flatten() #new index
        print('setAnSource::nodeYLeaf_newID ',nodeYLeaf_newID)        
        #self.D.constrain(0., where = leaves) no need? default face constrain is no flux
