...
class PhloemFluxPython(Leuning):
    ...
    def __init__(self, rs):
        ...
        self.rhoC = 0.07*0.4*0.4/12 #mol C /cm3 fresh matter
        ...
        self.phloemViscosity = 1.7e-3*1e-6*3600 #MPa hr-1 , cf Zhou 2020
        self.TairK = 293
        ...
        self.phiLim = 0
        self.lim = 0.1 #mmol C ml-3, see zhou et al. 2019
        self.VarMu = True
    ...
    def mp2mesh(self, TairK = 293, VariableMu = True): #TODO: adapt to make more time efficient. do it in c++?
        ...
        self.maxGrowth = np.array( [self.rs.organParam[organTypes[self.new2oldNodeID[xi] - 1]][subTypes[self.new2oldNodeID[xi] -1]+ 2*(organTypes[self.new2oldNodeID[xi] - 1]==4)].getParameter('r')/24 for xi in cells[1]]) #maxgrwth rate from cm/d to cm/s
    ...
    def _setOutflow(self): #only outflow when all segments have rm gr satisfaction?
        ...
        #==>
        self.outFlowMax =self.rootCells*(2.4e-05* (self.phi) * np.pi*self.radiiCells*2*self.mesh.length)/self.mesh.cellVolumes #mmol C hr-1 ml-1
    ...
    def _setintCoeff(self): #use to get right value of k/mu* RT
        ...
        if( self.VarMu):
            #mmol C to mol Suc
            self.VolFractSucrose = (self.phi.faceValue/12/1000)* sucroseMolarVolume #  #[mol/cm³] / [cm³/mol] = cm³/cm³ * sucroseMolarVolume
        else:
            #default value for piafmunch => 0.5 mmol C ml-1
            self.VolFractSucrose = (0.5e-3/12)* sucroseMolarVolume
        #valid for VolFractSucrose <= 0.65 <-> [sucrose] <= 0.003019281 mol/cm3
        self.phloemViscosity = waterViscosity * np.exp((4.68 * 0.956 * self.VolFractSucrose)/(1 - 0.956 * self.VolFractSucrose))* 1e-6 /( 60*60 )#Pa s-1 to MPa hr-1// #in [Pa.s = N-s/m^2]
        #mu = self.phloemViscosity 
        ...
        R =  self.Param['R']*1e-3  #MPa mL K-1 mmol-1 
        
        self.intCoeff = self.phloemConductivity/self.phloemViscosity * R * self.TairK # [cm/(MPa hr)]* R[MPa mL K-1 mmol-1] * T[K] = (cm mL)/(hr mol-1)  => added in divergence: C (mol ml-1) * A (cm2) / L (cm) => cm3/s
        
        # flow = (k/mu) * RT * d(C)/dx = [cm2]/[Pa s] * [Pa * mL /(K mol)] * K * d[mol/mL] 
        # = [cm2 Pa-1 s-1] * d[Pa ] = cm2 s-1 (?)

    def _setAnSource(self, simDuration,  Leuning = True): #sourceterm in leaf segment
        ...
        surfaceSide = (2*np.pi*((self.radiiCells))*(self.mesh.length)) #m2.
        if(Leuning):
            Ag = np.add(self.An , self.Rd)/10000*3600*1000#in mmol C cm-2 hr-1
        elif (simDuration >= 12 and simDuration < 24) :
            Ag = np.zeros(len(nodeYLeaf))
        else:
            Ag = np.full(len(nodeYLeaf),  6.40314e-004)
        ...
        for i, an in enumerate(Ag): #1.8e-11/self.mesh.cellVolumes in zhou2020
                ...
                s = 1
                if Leuning:
                    s = surfaceSide[self.mesh.cellFaceIDs[1] == nodeYLeaf_newID[i]] /2 
                    
                Cin =an*s/v #mmol C ml-1 hr-1
                self.Agross.constrain(Cin, where= self.mesh.cellFaceIDs[1] == nodeYLeaf_newID[i]) #(sucrose* surfaceSide /self.mesh.cellVolumes, where= self.mesh.cellFaceIDs[1] == nodeYLeaf_newID[i]) #0 node = y node for stem
        self.JS_Mesophyll = np.maximum(1.33e-5*((self.Mesophyll-self.phi))*leafCells,self.phi*0)/self.mesh.cellVolumes #from Uys2020 but with lower k coef to avoid error, 1e-1*(self.Mesophyll-self.phi)/(1+self.Mesophyll+self.phi) 
        #print("self.Agross ",self.Agross)
        
    def _setRmSink(self): # Daudet 2002: Rm = (k1 + k2 * C)Sr. 
        self.RmMax = ((self.rhoC*1000) * self.volumesSEG *((self.phi) * self.k1 + self.k2))/self.mesh.cellVolumes
    ...
    def _setGrSink(self): #Rg + C allocation = Grsink = growth * (1/GrowEff)
        ...
        maxCNeeded = (((self.maxGrowth*np.pi * (self.radiiCells**2))*self.rhoC *1000 /self.GrEff)) #mmol hr-1
        ...
        self.GrMax =  growthCells*maxCNeeded *  rxFactor * self.lengthFactor * leafSegLength/self.mesh.cellVolumes# * self.CSat 
        #print('GrMax ,',self.GrMax*self.mesh.cellVolumes)
    ...
    def getCLimitedSinks(self, sink_dot, dt):
        ...
        self.Growth0 = self.GrSink/1000 * self.mesh.cellVolumes / self.rhoC * self.GrEff #get increase in cm³ hr-1
    ...
    def resetValues(self, simDuration, Leuning = True):
        self._setAnSource(simDuration, Leuning)
        self._setintCoeff()
        self._setRmSink()
        self._setGrSink()
        self._setOutflow()
        ...
        self.Ufact = np.maximum((np.minimum(self.phi, 0.3) -self.lim)/(0.3 - self.lim), self.phi*0 )
    ...
    def valueError(self):
        if(any(self.phi > 3)): #risk of fraction phi going over 0.65
            ...
            raise Exception("phl.phi > 3 mmol/mL-1")
        if(any(self.phi < 0)):
...
def GetTime(hours):
    sec = timedelta(seconds=int(hours*3600))
