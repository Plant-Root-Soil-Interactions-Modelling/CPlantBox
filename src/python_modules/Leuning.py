...
class Leuning(XylemFluxPython):
    ...
    def calcAn( self,Q_input, Tl_input, log=True ):
        ...
        for si in range(len(length_leaf)) :
            ...
            '''Vcmax according to ruisco content
    ...
    def calcGs( self,VPD_input, p_l_input, Tl_input):
        ...
        for si in range(len(self.get_segments_index(4))) :
            ...
            #beta= self.Param['Patm']/(self.Param['R']*Tl)
            #coef = self.Param['Mh2o']/(self.Param['rho_h2o']*self.Param['R']*Tl)
            #trans = coef*(1/(beta/gw+self.grbvw[si]))* gw * VPDL* 24*3600e-4 #in cm3 cm-2 d-1
    ...
    def getNc(self):
        ...
        self.Nc = Nc
    '''
    def calcGbl(self, Tl_input,V_input, log ):
        D0co2 = 12.9*1e-6
        D0w = 21.2*1e-6
        self.grbvw = []
        self.grbvco2 = []
        ...
        for si in range(len(self.get_segments_index(4))) :
            if(isinstance(Tl_input,(float,int))):
                ...
                Tl = Tl_input[si]
            if(isinstance(V_input,(float,int))):
                V = V_input
            else:
                V = V_input[si]
            V = max(min(V, 30000),1)#equation valid for V in [1,30000]
            if(V <= 200):
                b = 0.58*exp(1/sqrt(radii_leaf[si]*V)-1)
            else:
                b = 0.58*sqrt(radii_leaf[si]/V)
            b = b/100 #cm=>m
            DT = D0co2 *(1+0.007*(Tl -273.15))#m s-1
            grbvco2.append(DT/b)
            DT = D0w *(1+0.007*(Tl -273.15))#m s-1
            grbvx.append(DT/b)
        self.grbvw = grbvx
        self.grbvco2 = grbvco2
    '''        
    def calcCi(self):
        ...
        for si in range(len(self.get_segments_index(4))) :
            ...
            if (self.fw[si] == self.Param['fwr']): #or (self.An[si] == 0):
                ci = 0.9*self.deltagco2[si]
            else: 
                ci = self.Param['cs'] - self.An[si]/self.gco2[si]  #Eq 26
                ci = min(max(ci,0.9*self.deltagco2[si]),  self.Param['cs'])
            '''
            ci = (self.Param['cs']*self.Param['a1']*self.fw[si] +self.deltagco2[si])/(1+self.Param['a1']*self.fw[si]) #Eq 26
            #print(ci,self.An[si], self.gco2[si] ,min(max(ci,0.9*self.deltagco2[si]),  self.Param['cs'])  )
            #ci = min(max(ci,0.9*self.deltagco2[si]),  self.Param['cs'])
            self.ci.append(ci)
