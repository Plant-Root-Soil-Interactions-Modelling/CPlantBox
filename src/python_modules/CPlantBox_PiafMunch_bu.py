...
def write_PiafMunch_parameter(plant, Soil_water = -100, k1 = 0, name='mg_test.ini', time=10, nodes_r_xyl = 0.005, vml = 0.000143136, vol_st =2.6e-05, vmu = 2.82627e+95, r_phl_mb = 0.027157 , r_trsv = 100/5000): #function of creating parameters of PiafMunch
	...
	#'******** INITIAL VALUES *********\n'
	#initialization of the parameters
	Q_ST = 1#np.full(len(nodes_organtype), 0)
	Q_Sympl =0# np.full(len(nodes_organtype), 4.4e-006)
	Starch =0# np.full(len(nodes_organtype), 1)
	Q_PhlApo = 0#np.full(len(nodes_organtype), 4.4e-006)
	Q_ParApo = 0#np.full(len(nodes_organtype), 4.4e-006)
	Tr_Q_ST = 0#np.full(len(nodes_organtype), 0)
	Tr_Q_Sympl = 0#np.full(len(nodes_organtype), 4.4e-006)
	Tr_Starch = 0#np.full(len(nodes_organtype), 1)
	Tr_Q_PhlApo = 0#np.full(len(nodes_organtype), 0)
	Tr_Q_ParApo = 0#np.full(len(nodes_organtype), 0)
	vol_Sympl =0# np.full(len(nodes_organtype), 2.6e-005)
	...
	Q_ST_Abs =  1e-015
	Q_Sympl_Abs = 0# 1e-015
	Starch_Abs =  0#1e-012
	Q_PhlApo_Abs =  0#1e-015
	Q_ParApo_Abs =  0#1e-015
	Tr_Q_ST_Abs =  1e-012
	Tr_Q_Sympl_Abs =  0#1e-012
	Tr_Starch_Abs =  0#1e-012
	Tr_Q_PhlApo_Abs =  0#1e-015
	Tr_Q_ParApo_Abs =  0#1e-015
	vol_Sympl_Abs = 0#1e-012
	...
	f.write("node#\tupfl.node\tdnfl.node\tr_Xyl\tr_ST\n")
	...
	for i, ot in enumerate(organTypes):
		...
		print(surfaces[i+1],plant.phloemViscosity* 1e-6 /( 60*60 ),mu/ k, f'{i + 1}\t{upNode + 1}\t{downNode + 1}\t{r_Xyl}\t{r_ST}\n')
	f.write('\n')

	f.write('Individual Node : Lateral Resistances (MPa h / ml)\n')
	f.write("{:s}\t{:s}\t{:s}\t{:s}\t{:s}\t{:s}\n".format('node#','r_Trsv','r_PhlMb','r_ParMb','r_Apo', 'r_Sympl' ))
	for i, j in enumerate(organTypes):#check for  trsv and phlmb
		f.write(f'{ 1+i}\t{125.}\t{125.}\t{1e25}\t{1e25}\t{1e25}\n')#.format(nodes_organtype[i][0],r_Trsv[i],r_PhlMb[i],r_ParMb[i],r_Apo[i],r_Sympl[i]))
	f.write('\n')#replace one to represent outflow?
