...
public:
    ...
    Stem(Organism* rs, int type, Vector3d pheading, double delay, Organ* parent, double pbl, int pni); ///< used within simulation
    virtual ~Stem() { }; ///< no need to do anything, children are deleted in ~Organ()
    ...
    double dx() const { return getStemRandomParameter()->dx; } ///< returns the axial resolution
