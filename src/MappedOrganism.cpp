...
void MappedSegments::mapSegments(const std::vector<Vector2i>& segs) {
	...
	if (skipped_segment == 1){
		std::cout << "MappedSegments::mapSegments: segments with mid " << std::flush; 
		for (int i = 0; i < vec.size(); i++) {
			std::cout << vec[i].toString() << " "<< std::flush; 
		}
		std::cout <<  "exceeds domain, skipped segment \n"<< std::flush;
	}
}
...
void MappedRootSystem::simulate(double dt, bool verbose)
{
    ...
    auto newsegO = this->getNewSegmentOrigins(); // to add radius and type (TODO cutting) why origin? seg type define by end node
    radii.resize(radii.size()+newsegO.size());
    types.resize(types.size()+newsegO.size());
    typesorgan.resize(typesorgan.size()+newsegO.size());
    c = 0;
