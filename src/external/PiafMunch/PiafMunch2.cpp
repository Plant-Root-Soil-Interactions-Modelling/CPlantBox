...
double* Q_S_ST = NULL						; // Amount of sugar in sieve tubes	= C_TC * Vol_ST						(mmol)
...
double * Q_Mesophyll_dot = NULL, * Q_ST_dot = NULL,* Q_S_Mesophyll_dot = NULL, * Q_S_ST_dot = NULL, * Q_Rm_dot = NULL, *Q_Exud_dot = NULL, *Q_Gtot_dot = NULL,  *Q_out_dot = NULL  ;
...
void PhloemFlux::C_fluxes(double t, int Nt)  
{
	...
	{ // edit (make different loops) to enter specific equations for specific nodes or conn.orders
		...
		Q_S_ST_dot[i] = StarchSyn + Starch_dot_alt - kHyd_S_ST * Q_S_ST[i]; // (Vmax and kHyd) or k3 (below),  or all three, should be zero
