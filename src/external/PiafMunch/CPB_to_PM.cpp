...
void CPB_to_PM::updateBudStage(double dt)// TODO: check at each time step
{
    ...
    for(auto org: orgs)
	{
		if(org->getBudStage() != 2)//org->getOrganRandomParameter()->isBudType)
		{
			...
			{ org->setBudStageChange(org->getBudStage(),dt);}
...
double CPB_to_PM::getMaxVolumicGrowth(std::shared_ptr<CPlantBox::Organ> org, double t, double rmax, double Lmax)
{
	...
	if((e + org->getLength(false))> Lmax + 1e-10){
		...
		std::cout<<" k: "<<org->getParameter("k")<< " " << Lmax <<" orgid: "<<org->getId()<<std::endl;
		assert(false);
	}
	...
	if(org->organType() == 3){
		...
		assert(((org->budStage !=1)||(Linit - maxLBud_ < 1e-10))&&"!(((budStage !=1)||(length - maxLBud_ < 1e-10)))");
		assert(((org->budStage !=0)||(Linit - maxLBudDormant_< 1e-10))&&"!(((budStage !=0)||(length < 1e-10)))");
		assert(((org->budStage !=1)||(Linit + e - maxLBud_ < 1e-10))&&"!(((budStage !=1)||(length + dl - maxLBud < 1e-10)))");
		assert(((org->budStage !=0)||(Linit + e - maxLBudDormant_< 1e-10))&&"!(((budStage !=0)||(length + dl - maxLBudDormant < 1e-10)))");
	}
