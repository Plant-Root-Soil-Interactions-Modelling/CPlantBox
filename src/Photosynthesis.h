...
public:
	...
	//			put in files of phloem module
    std::vector<std::vector<double>> kr_suc; //  [mmol Suc /hPa day-1]
    std::vector<std::vector<double>> kx_suc; // [mmol Suc day-1]
    std::vector<std::vector<double>> a_ST_eqs; // [mmol Suc day-1]
	double kx_suc_f(int type, int organType) {return kx_suc.at(organType - 2).at(type-1); }//per subtype and organ type (goes from 2 (root) to 4 (leaf))
    double kr_suc_f(int type, int organType) {return kr_suc.at(organType - 2).at(type-1); }//per subtype and organ type (goes from 2 (root) to 4 (leaf))
    double get_a_ST_eq( int type, int organType){return a_ST_eqs.at(organType - 2).at(type-1); }
