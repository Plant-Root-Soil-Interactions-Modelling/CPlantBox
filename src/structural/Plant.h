...
class Plant : public Organism {
  public:
    ...
    void simulateLimited(double dt, double max_, std::string paramName, std::vector<double> scales, std::shared_ptr<ProportionalElongation> se,
                         bool verbose); ///< simulates plant with limited costs
    ...
    std::shared_ptr<Tropism> createTropismFunction(int tt, double N, double sigma,
                                                   double ageSwitch = 0.); ///< Creates the tropisms, overwrite or change this method to add more tropisms
    virtual std::shared_ptr<GrowthFunction>
    createGrowthFunction(int gft); ///< Creates the growth function per root type, overwrite or change this method to add more tropisms
