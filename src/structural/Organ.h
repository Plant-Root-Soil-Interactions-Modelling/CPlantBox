...
/**
 * @brief Base class for plant organs (seed, root, stem, leaf).
 *
 * Stores organ state, geometry, parent-child relations, and shared post-processing helpers.
 */
class Organ : public std::enable_shared_from_this<Organ> {

  public:
    Organ(int id, std::shared_ptr<const OrganSpecificParameter> param, bool alive, bool active, double age, double length, Vector3d partialIHeading_, int pni,
          bool moved = false, int oldNON = 0); ///< Constructs an organ from explicit state.
    Organ(std::shared_ptr<Organism> plant, std::shared_ptr<Organ> parent, int organtype, int subtype, double delay,
          int pni); ///< Constructor used during simulation.
    virtual ~Organ() {}
    ...
    std::shared_ptr<const OrganSpecificParameter> param() const { return param_; } ///< specific organ parameters
    std::shared_ptr<OrganRandomParameter> getOrganRandomParameter() const;         ///< organ type random parameters given by mean and std
    bool isAlive() const { return alive; }                                         ///< checks if organ is alive
    bool isActive() const { return active; }                                       ///< checks if organ is active
    ...
    virtual double calcLength(double age) { throw std::runtime_error("calcLength() not implemented"); }    ///< Inverse mapping age -> length.
    ...
    void rel2abs();                                    ///< Converts node coordinates from relative to absolute.
    void abs2rel();                                    ///< Converts node coordinates from absolute to relative.
    void moveOrigin(int idx);                          ///< Updates parent-link node index, e.g. during nodal growth.
    ...
    bool hasRelCoord() const;              ///< True if nodes are currently stored in relative coordinates.
    bool has_rel_coord = false;

    int parentNI; ///< Local parent node index where this organ is attached.
  ...
  protected:
    mutable Vector3d partialIHeading; ///< Initial local heading; mutable for derived heading updates.
    ...
    virtual Vector3d getIncrement(const Vector3d &p, double sdx,
                                  int n = -1); ///< called by createSegments, to determine growth direction. overriden by @see Leaf::getIncrement()
    void createSegments(double l, double dt, bool silence, int phytoIdx = -1); ///< creates segments of length l, called by Root::simulate()
    virtual double getLatInitialGrowth(double dt);                             ///< Initial growth duration passed to a new lateral.
    virtual double getLatGrowthDelay(int ot_lat, int st_lat, double dt) const; ///< Delay before a new lateral starts growing.
    bool getApplyHere(int i) const;                                            ///< Checks whether successor rule i applies at current node.
    std::vector<int> addIncrement;                                             ///< Helper indices used while switching coordinate systems.
