...
class MappedPlantPython:
    ...
    def read_rsml(file_name: str, verbose=True):
        ...
        if bn > 1:
            ...
            if verbose:
                print("MappedPlantPython.read_rsml: added an artificial shoot")
        nodes, segs = rsml.get_segments(polylines, props)
        if verbose:
            print("MappedPlantPython.read_rsml: read rsml with", len(nodes), "nodes and", len(segs), "segments")
