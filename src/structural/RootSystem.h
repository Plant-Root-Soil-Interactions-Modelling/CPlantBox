...
class RootSystem : public Organism {
  public:
    ...
    void simulate(double dt, double maxinc, std::shared_ptr<ProportionalElongation> se,
                  bool verbose = true); ///< simulates the root system with a maximal overall elongation
    ...
    int getNumberOfRoots(bool all = false) const {
        if (all)
            return organId + 1;
        else
