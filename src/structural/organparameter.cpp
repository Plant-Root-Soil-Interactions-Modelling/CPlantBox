        ...
        if (str != nullptr) {
            ...
            if (i == 0) {
                if ((key.compare("leafGeometry") != 0) || (organType != Organism::ot_leaf)) {
                    if (verbose) {
                        std::cout << "OrganRandomParameter::readXML: warning! parameter " << key << " is defined in the xml, but not available in organ "
                                  << Organism::organTypeName(organType) << "\n"
                                  << std::flush;
                    }
                }
            }
...
/**
 * @brief Reads a single successor rule from an XML element.
 *
 * Parses the attributes @c ruleId (or legacy @c number), @c numLat, @c where,
 * @c subType (or legacy @c type), @c organType, and @c probability (or legacy
 * @c percentage) and stores them in the corresponding successor vectors
 * (successorOT, successorST, successorP, successorNo, successorWhere).
 * New rule slots are created as needed.
 *
 * @param p        XML element with attribute @c name="successor".
 * @param verbose  If true, print diagnostic messages for missing optional attributes.
 */
void OrganRandomParameter::readSuccessor(tinyxml2::XMLElement *p, bool verbose) {
    ...
    if (key.compare("successor") == 0) {
        ...
        if (success != tinyxml2::XML_SUCCESS) {
            ...
            if (success != tinyxml2::XML_SUCCESS) {
                if (verbose) {
                    std::cout << "OrganRandomParameter::readSuccessor: for parameter of organ " + std::to_string(organType) + ", subType " +
                                     std::to_string(subType) + ", 'ruleId' (and 'number') not found in successor definition. ";
        ...
        cpb_queryStringAttribute(lookfor, defaultVald, defaultSize, replaceByDefaultValue, successorWhere.at(ruleId),
                                 p); // name, default value, vector to fill, accept not found

        replaceByDefaultValue = false;
        lookfor = std::vector<std::string>{"subType", "subtype", "type"}; // subtype (or type for backarwad compatibility)
        defaultVal = 1.0;
        defaultSize = 0;
        cpb_queryStringAttribute(lookfor, defaultVal, defaultSize, replaceByDefaultValue, successorST.at(ruleId), p);
        ...
        if (!std::isfinite(defaultVald)) {
            ...
            std::cout << std::endl;
            throw std::runtime_error("OrganRandomParameter::readSuccessor !std::isfinite(default growth probability) " + std::to_string(defaultVald) +
                                     " , successorST.at(ruleId).size() == " + std::to_string(successorST.at(ruleId).size()));
        }
        cpb_queryStringAttribute(lookfor, defaultVald, defaultSize, replaceByDefaultValue, successorP.at(ruleId), p);

        replaceByDefaultValue = true;
        lookfor = std::vector<std::string>{"organType", "organtype"};
        if ((successorST.at(ruleId).at(0) == 2) && (this->organType == Organism::ot_stem)) {
            ...
            defaultVal = this->organType;
        }
        defaultSize = (successorST.at(ruleId).size() - successorOT.at(ruleId).size());
        cpb_queryStringAttribute(lookfor, defaultVal, defaultSize, replaceByDefaultValue, successorOT.at(ruleId), p);
...
/**
 * @brief Queries a string XML attribute under one of several candidate names and parses it into a vector.
 *
 * Tries each name in @p keyNames in order.  On success the comma-separated attribute value is
 * parsed and appended to @p vToFill.  If no candidate is found and @p replaceByDefault is true,
 * @p sizeVector copies of @p defaultVal are appended instead; otherwise an assertion fires.
 *
 * @tparam IntOrDouble      Element type of the output vector (int or double).
 * @param keyNames          Ordered list of XML attribute names to try.
 * @param defaultVal        Default element value and type-selector for string2vector.
 * @param sizeVector        Number of default elements to insert when the attribute is absent.
 * @param replaceByDefault  Allow fallback to @p defaultVal when the attribute is missing.
 * @param vToFill           Output vector; values are appended.
 * @param key               XML element to query.
 */
template <class IntOrDouble>
void OrganRandomParameter::cpb_queryStringAttribute(std::vector<std::string> keyNames, IntOrDouble defaultVal, int sizeVector, bool replaceByDefault,
    ...
    if (tinyxml2::XML_SUCCESS != success) {
        assert(replaceByDefault && "mymath::queryStringAttribute: key not found in xml file without default value");
...
tinyxml2::XMLElement *OrganRandomParameter::writeXML(tinyxml2::XMLDocument &doc, bool comments) const {
    ...
    for (int j = 0; j < successorST.size(); j++) {
        ...
        if (successorNo.size() > j) {
            p->SetAttribute("numLat", successorNo.at(j));
        }
        if (successorWhere.size() > j) {
            p->SetAttribute("Where", vector2string(successorWhere.at(j)).c_str());
        }
        p->SetAttribute("subType", vector2string(successorST.at(j)).c_str());
        if (successorOT.size() > j) {
            p->SetAttribute("organType", vector2string(successorOT.at(j)).c_str());
        }
 ...
 *
 *  @param name   Parameter name used in XML, toString(), and getParameter().
 *  @param i      Pointer to the integer member variable.
 *  @param descr  Human-readable description; written as XML comment when comments=true.
 *  @param dev    Optional pointer to the parameter's deviation variable.
 */
 ...
 *
 *  @param name   Parameter name used in XML, toString(), and getParameter().
 *  @param d      Pointer to the double member variable.
 *  @param descr  Human-readable description; written as XML comment when comments=true.
 *  @param dev    Optional pointer to the parameter's deviation variable (e.g. standard deviation).
 */
...
/**
 * @brief Parses a comma-separated string from XML into a vector of int.
 * @param xmlInput   Null-terminated string read from an XML attribute.
 * @param defaultVal Unused; selects this int overload via overload resolution.
 * @return Parsed integer vector.
 */
...
/**
 * @brief Parses a comma-separated string from XML into a vector of double.
 * @param xmlInput   Null-terminated string read from an XML attribute.
 * @param defaultVal Unused; selects this double overload via overload resolution.
 * @return Parsed double vector.
 */
...
/**
 * @brief Converts an integer vector to a comma-separated string for XML serialisation.
...
/**
 * @brief Converts a double vector to a comma-separated string for XML serialisation.
 * @param vec  Vector to convert.
 * @return Comma-separated string representation.
 */
