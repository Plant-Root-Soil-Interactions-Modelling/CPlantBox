...
} // needed for boost python indexing suite
...
class Matrix3d {
  public:
    ...
    Vector3d column(int i) const {
        ...
        switch (i) {
        case 0:
            return Vector3d(r0.x, r1.x, r2.x);
        case 1:
            return Vector3d(r0.y, r1.y, r2.y);
        case 2:
            return Vector3d(r0.z, r1.z, r2.z);
        }
    ...
    Vector3d times(const Vector3d &v) const { return Vector3d(r0.times(v), r1.times(v), r2.times(v)); } ///<  Multiplies vector v from right
...
class Turtle3D {
  public:
    ...
    Turtle3D() : pos(0., 0., 0.), frame(Matrix3d(1, 0, 0, 0, 1, 0, 0, 0, 1)) {}

    /// Initialises with a given position and frame.
    Turtle3D(const Vector3d &position, const Matrix3d &frame) : pos(position), frame(frame) {}
    ...
    std::string toString() const {
        std::ostringstream s;
        s << "pos=" << pos.toString() << " H=" << heading().toString() << " L=" << left().toString() << " U=" << up().toString();
        return s.str();
    }
  ...
  private:
    ...
    void rotateAroundColumn(int axis, double angle) {
        ...
        for (int c = 0; c < 3; c++) {
            if (c == axis)
                continue; // axis column is unchanged
