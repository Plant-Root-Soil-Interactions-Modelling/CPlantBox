...
void Plant::setTropism(std::shared_ptr<Tropism> tf, int organType, int subType) // todo
{
    ...
    for (auto &orp_ : orp)
        ...
        default:
            throw std::invalid_argument("Plant::setTropism() cannot set tropism for organ type " + Organism::organTypeName(organType));
        }
}
 ...
 * @param verbose       indicates if status is written to the console (cout) (default = false)
 */
void Plant::simulateLimited(double dt, double max_, std::string paramName, std::vector<double> scales, std::shared_ptr<ProportionalElongation> se,
                            bool verbose) {
    ...
    if ((inc_ > maxinc) && (std::abs(inc_ - maxinc) > accuracy)) { // if necessary, perform a binary search
        ...
        while (((std::abs(inc_ - maxinc)) > accuracy) && (i < maxiter)) { // binary search
            ...
            if (verbose) {
                std::cout << i << "\t(sl, mid, sr) = (" << sl << ", " << m << ", " << sr << "), inc " << inc_ << ", err: " << std::abs(inc_ - maxinc) << "<>"
                          << accuracy << "\n";
            }
...
std::shared_ptr<Tropism> Plant::createTropismFunction(int tt, double N, double sigma, double ageSwitch) {
    switch (tt) {
    case tt_plagio:
        return std::make_shared<Plagiotropism>(shared_from_this(), N, sigma);
    case tt_gravi:
        return std::make_shared<Gravitropism>(shared_from_this(), N, sigma);
    case tt_exo:
        return std::make_shared<Exotropism>(shared_from_this(), N, sigma);
    ...
    case tt_twist:
        return std::make_shared<TwistTropism>(shared_from_this(), N, sigma);
    case tt_antigravi:
        return std::make_shared<AntiGravitropism>(shared_from_this(), N, sigma);
    case tt_antigravi2gravi:
        return std::make_shared<AntiGravi2Gravitropism>(shared_from_this(), N, sigma, ageSwitch);
    default:
        throw std::invalid_argument("Plant::createTropismFunction() tropism type not implemented");
    }
}
...
std::shared_ptr<GrowthFunction> Plant::createGrowthFunction(int gft) {
    switch (gft) {
    case gft_negexp:
        return std::make_shared<ExponentialGrowth>();
    case gft_linear:
        return std::make_shared<LinearGrowth>();
    case gft_CWLim:
        return std::make_shared<CWLimitedGrowth>();
    default:
        throw std::invalid_argument("Plant::createGrowthFunction() growth function type not implemented");
    }
}
 ...
 */
std::string Plant::toString() const { return "Plant " + Organism::toString(); }
