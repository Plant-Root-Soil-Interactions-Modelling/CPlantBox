...
void Photosynthesis::toFile(std::string fileName,std::vector<double>& descriptorsValues)
{
	...
	data_file.open (outputDir+fileName, std::ios_base::app | std::ios_base::in);
	...
	/* solves the coupled water flux + carbon assimilation and stomatal opening, 
		...
		@param ea [hPa]                	relative ari humidity
		@param TairC_ [°C]               leaf temperature (mean)     
	*/
	
void Photosynthesis::solve_photosynthesis(double ea_, double es_, double sim_time_,std::vector<double> sxx_, bool cells_ , 
	 std::vector<double> soil_k_, bool doLog_ , int verbose_ , double TairC_, std::string outputDir_)
{
	...
	An.resize( seg_leaves_idx.size(), 0.);
	gco2.resize( seg_leaves_idx.size(), 0.);
	ci.resize( seg_leaves_idx.size(), ciInit);
	pg.resize( seg_leaves_idx.size(), psiXylInit);
	...
	if(followTrace)
        {
            std::fstream myfile1;
            myfile1.open("loopPg.csv");myfile1.close();
            myfile1.open("loopCi.csv");myfile1.close();
            myfile1.open("loopAn.csv");myfile1.close();
            myfile1.open("loopVc.csv");myfile1.close();
            myfile1.open("loopVj.csv");myfile1.close();
            myfile1.open("loopGco2.csv");myfile1.close();
            myfile1.open("loopFw.csv");myfile1.close();
            myfile1.open("loopEv.csv");myfile1.close();
        }
	while(!this->stop){  
        ...
        {            
            ...
            toFile("loopVc.csv",Vc);
            toFile("loopVj.csv",Vj);
		...
		if((verbose_photosynthesis > 0)){
            ...
            std::cout<<"(loop > maxLoop): "<<(loop > maxLoop)<<", (maxMaxErr < "<<limMaxErr<<"): "<<(maxMaxErr < limMaxErr)<<" (loop>minLoop): "<<(loop>minLoop)<<std::endl;
		}
...
void Photosynthesis::getError(double simTime)
{
	...
	{ 
		...
		if(this->psiXyl[i]>0){
			std::cout<< "Photosynthesis xyl >0, "<<i<<" "<<this->psiXyl[i]<<std::endl<<std::flush;
	...
	{ //f = f < 0 ? -f : f;
		...
		maxErr[8] +=0;// std::abs((this->An[i]-An_old[i])/tempVal);
...
void Photosynthesis::loopCalcs(double simTime){
	...
	for(int i = 0; i<seg_leaves_idx.size();i++)
	{
		...
		if(std::abs(fv.at(i)) > 1e-16)//i.e., perimeter * kr > 1e-16 like for @see Xylem::solveLinear
		{
            ...
            {
                throw std::runtime_error("both DoSun and DoWiggle turned on");
            }
            
            fwtemp = fw.at(i);
            if(fwWiggle&&doWiggle&&(loop>10))
            {
                if(doLog ){
                    myfile4<<"fwWiggle fwtemp:"<<fw.at(i)<<", fwold:" <<fw_old.at(i)<<", fwVold:"<<fw_very_old.at(i);
                }
                fw.at(i) = (fw_old.at(i)+fw_very_old.at(i))/2;
                if(doLog ){myfile4<<", fwnew:" <<fw.at(i)<<std::endl; }
            }
            if(DoSun2012&&(fw_old.at(i) >=0.)&&(fw_very_old.at(i) >=0.)&&(loop>10))
            {
                if(doLog ){
                    myfile4<<"DoSun2012 fwtemp:"<<fw.at(i)<<", fwold:" <<fw_old.at(i)<<", fwVold:"<<fw_very_old.at(i);
                }
                fw.at(i) = fw_very_old.at(i) -std::pow((fw_old.at(i) - fw_very_old.at(i)),2.)/(fw.at(i) -2*fw_old.at(i)+ fw_very_old.at(i));
                fw_old.at(i) = -1.;//throw away guesses
                fw_very_old.at(i)=-1;
                if(doLog ){myfile4<<", fwnew:" <<fw.at(i)<<std::endl; }
            }
            ...
            if(oldciEq)
            {
                ci.at(i) = std::max((cs*a1*fw.at(i) +deltagco2.at(i))/(1+a1* fw.at(i)),deltagco2.at(i)+cs/100) ;//Eq 26	
            }else{
                R_eq_co2 = (1/(gco2.at(i) ) + 1/(g_bl) + 1/(g_canopy ) + 1/(g_air )) ;
                ci.at(i) = std::max(deltagco2.at(i)+cs/100,cs - (An.at(i) - Rd) *R_eq_co2) ;// g_eq_co2;
            }
            ...
            if(!useVj)
            {
                An.at(i) = Vc.at(i)- Rd;//Eq 6
            }
            if(!useVc)
            {
                An.at(i) = Vj.at(i)- Rd;//Eq 6
            }
            if(alternativeAn)
            {
                //cf Bonance 2019 and Collatz et al. 1990
                //Theta_ag*Ag*Ag - (Vc.at(i) + Vj.at(i))*Ag + Vc.at(i)* Vj.at(i) = 0.; 
                double coefa = Theta_ag;
                double coefb = -(Vc.at(i) + Vj.at(i));
                double coefc =Vc.at(i)* Vj.at(i);
                if(Theta_ag ==0)
                {
                     An.at(i) =(Vc.at(i)* Vj.at(i))/(Vc.at(i) + Vj.at(i));
                }else{
                double dis = std::pow(coefb,2.) - (4.*coefa*coefc);
        if (dis < 0) {
				throw std::runtime_error("Photosynthesis::loopCalcs : root for Ag not found");
          }
                An.at(i) =  ((-coefb- std::sqrt(dis))/(2.*coefa));//Ag
        if (An.at(i) < 0) {
				throw std::runtime_error("Photosynthesis::loopCalcs : Ag < 0");
          }
                }
                An.at(i) = An.at(i) - Rd;//
		...
		if(doLog ){
			...
			myfile4<<"an calc "<<alternativeAn<<" "<<An.at(i)<<" "<<Vc.at(i)<<" "<< Vj.at(i)<<" "<<J<<" "<<Vcmax.at(i)<<" "<<Kc<<" "<<Ko<<" ";
			...
			myfile4<<"diff Ev and lat fluw: "<<Ev.at(i)<<std::endl;//<<" "<<outputFluxL.at(i)
                myfile4<<"fw.at(i) "<<fw.at(i)<<" fwtemp "<<fwtemp<<" fwold:"<<fw_old.at(i)<<" very old "<<fw_very_old.at(i)<<" doWhat"<<doWiggle<<" "<<DoSun2012<<(loop>3)<<" "<<(loop>10)<<std::endl;
		}
	}
