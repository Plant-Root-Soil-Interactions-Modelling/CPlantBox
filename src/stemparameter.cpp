...
std::shared_ptr<OrganSpecificParameter> StemRandomParameter::realize()
{
	...
	if (!hasLaterals) { // no laterals
		...
		if((res < dxMin) && (res != 0)){
			...
			//this->lb=lb_; otherwise get wrong nob_real value for the next stems
		}	
		res = la_-floor(la_ / dx)*dx;	
		if(res < dxMin && res != 0){
			if(res <= dxMin/2){ la_ -= res;
			}else{la_ =  floor(la_ / dx)*dx + dxMin;}
			//this->la=la_; otherwise get wrong nob_real value for the next stems
		}	
		double ln_mean = ln;
		...
		switch(lnf) 
		{
			...
			for (int i = 0; i<nob_*2-1; i++) { // create inter-stem distances
				double d =  std::max(ln_mean*(1+i) +p->randn()*lns,dxMin); //std::max(  );//ln +p->randn()*lns,1e-9);
				res = d -floor(d / dx)*dx;
				if(res < dxMin && res != 0){
					if(res <= dxMin/2){d -= res;
					}else{d = floor(d / dx)*dx + dxMin;}
			...
			for (int i = 0; i<nob_-1; i++) { // create inter-stem distances
				double d =  std::max(ln_mean +p->randn()*lns,dxMin); //std::max(  );//ln +p->randn()*lns,1e-9);
				res = d -floor(d / dx)*dx;
				if(res < dxMin && res != 0){
					if(res <= dxMin/2){d -= res;
					}else{d = floor(d / dx)*dx + dxMin;}
