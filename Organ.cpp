 ...
 */
Organ::Organ(Plant* rs, int type, Vector3d pheading, double delay,  Organ* parent, double pbl, int pni)
{
 ...
 */
double Organ::getCreationTime(double length)
{
	assert(length>=0);
 ...
 */
double Organ::getLength(double age)
{
	assert(age>=0);
 ...
 * @param length   length of the root [cm]
 */
double Organ::getAge(double length)
{
	assert(length>=0);
	return rootsystem->gf.at(param.type-1)->getAge(length,param.r,param.getK(),this);
}

RootTypeParameter* Organ::getRootTypeParameter() const
{
 ...
 */
void Organ::createLateral(bool silence)
{
 ...
 */
void Organ::createSegments(double l, bool silence)
{
 ...
 */
std::vector<Organ*> Organ::getRoots()
{
 ...
 */
void Organ::getRoots(std::vector<Organ*>& v)
{
