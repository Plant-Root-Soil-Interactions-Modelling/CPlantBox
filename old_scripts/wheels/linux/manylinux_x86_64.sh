...
OUT_DIR="wheelhouse/linux/manylinux2014_x86_64"
mkdir -p "$OUT_DIR"

log_info "[manylinux x86_64] Pulling $IMAGE..."
docker pull "$IMAGE" >/dev/null

log_info "[manylinux x86_64] Building + repairing wheels (cp39-cp312)"
docker run --rm -t \
  --platform "$PLATFORM" \
  ...
  "$IMAGE" bash -lc $'\
    ...
    for PYBIN in /opt/python/cp3{9,10,11,12}-*/bin; do\n\
      ...
      echo "[manylinux x86_64] Building for $PY"\n\
      cd /project\n\
      ...
      echo "[manylinux x86_64] Smoke test for $PY"\n\
