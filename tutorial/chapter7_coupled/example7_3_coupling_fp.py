...
for i in range(0, n_steps):  # |\label{l73c:loop}|
    ...
    print(f"[{'*' * n}{' ' * (100 - n)}], {c:g} iterations, soil hs [{np.min(hs):g}, {np.max(hs):g}], interface [{np.min(hsr):g}, {np.max(hsr):g}] cm, root [{np.min(hx):g}, {np.max(hx):g}] cm, {s.sim_time:g} days")
