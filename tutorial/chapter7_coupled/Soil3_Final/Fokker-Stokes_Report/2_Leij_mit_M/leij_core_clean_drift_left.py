...
def analytical_solution_rdep(
    ...
    *,
    ...
    drift_sign: int = -1,   # NEW: -1 = leftward drift (default, compaction); +1 = rightward
) -> np.ndarray:
    """
    ...
    ----------
    drift_sign : {-1, +1}
        Sign of the drift in ln r. Use -1 (default) for compaction (shift to finer pores).
        Use +1 if you want coarsening (shift to larger pores).

    Toggle:
      xi_power_on_term2=False -> corrected form (ξ^A only on erfc term)
    ...
    # ---- DRIFT: flip sign here (default -1 → compaction/left shift) ----
    drift_sign = -1 if drift_sign not in (-1, +1) else drift_sign
    mu = drift_sign * (1.0 - eps0) * T
    ...
    out = np.empty_like(r, dtype=float)
    mass_fac = math.exp(M_over_u0w * T)  # keep sign as-is (controls φ path)

    xi_pow = np.exp(Aexp * np.log(xis))  # ξ^A

    for i, rr0 in enumerate(r):
        rr = float(max(rr0, a))
        # Note: with mu negative, centers move left in ln r as T increases
