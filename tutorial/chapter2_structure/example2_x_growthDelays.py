...
plant = pb.MappedPlant(2)  #|\label{l2x:plantStart}|
plant.readParameters(path + "example2_x.xml")
srp = plant.getOrganRandomParameter(pb.seed)[0]

srp.delayDefinition      = 0  # laterals wait for the apical zone to pass  |\label{l2x:ddRoot}|
srp.delayDefinitionShoot = 0  # same rule for shoot laterals                |\label{l2x:ddShoot}|

plant.initialize(False)  #|\label{l2x:init}|
...
tap_root = plant.getOrgans(pb.root)[0] #|\label{l2x:analyticald}|
...
growthDelay = ageLG - ageLN # time the lateral has to wait #|\label{l2x:analyticaldend}|
...
srp = plant.getOrganRandomParameter(pb.seed)[0]
srp.delayDefinition      = 1  # parent decides delay for root laterals    |\label{l2x:ddRootB}|
srp.delayDefinitionShoot = 1  # parent decides delay for shoot laterals   |\label{l2x:ddShootB}|
...
rrp.ldelay  = 2.5  # mean lateral delay [days]   |\label{l2x:root_ldelayB}|
rrp.ldelays = 0.   # std dev of delay  [days]    |\label{l2x:root_ldelaysB}|


# Stem subType 1 (main stem): shoot laterals wait 5 days.
srp = plant.getOrganRandomParameter(pb.stem)[1]
srp.ldelay  = 2.5   # |\label{l2x:stem_ldelayB}|
srp.ldelays = 0.    # |\label{l2x:stem_ldelayB}|
...
srp = plant.getOrganRandomParameter(pb.seed)[0]
srp.delayDefinition      = 0  # roots: use distance-based delay       |\label{l2x:ddRootC}|
srp.delayDefinitionShoot = 2  # shoot: lateral decides its own delay       |\label{l2x:ddShootC}|



# Leaf subType 1: shoot laterals wait 5 days.
lrp = plant.getOrganRandomParameter(pb.leaf)[1]
lrp.ldelay  = 2.5  # |\label{l2x:leaf_ldelayC}|
lrp.ldelays = 0.   # |\label{l2x:leaf_ldelaysC}|
