...
plant = pb.MappedPlant(2)  #|\label{l2_1_2:plantStart}|
plant.readParameters(path + "example2_1_2.xml")
srp = plant.getOrganRandomParameter(pb.seed)[0]

srp.delayDefinition      = 0  # laterals wait for the apical zone to pass  |\label{l2_1_2:ddRoot}|
srp.delayDefinitionShoot = 0  # same rule for shoot laterals                |\label{l2_1_2:ddShoot}|

plant.initialize(False)  #|\label{l2_1_2:init}|

anim = va.AnimateRoots(plant) #|\label{l2_1_2:growth}|
anim.min = min_
anim.max = max_
anim.res = [1, 1, 1]
anim.plant = True
...
for i in range(0, N):
    ...
    anim.update()
vp.plot_plant(plant, "creationTime")  #|\label{l2_1_2:growthEnd}|

tap_root = plant.getOrgans(pb.root)[0] #|\label{l2_1_2:analyticald}|
...
growthDelay = ageLG - ageLN # time the lateral has to wait #|\label{l2_1_2:analyticaldend}|
...
srp = plant.getOrganRandomParameter(pb.seed)[0]
srp.delayDefinition      = 1  # parent decides delay for root laterals    |\label{l2_1_2:ddRootB}|
srp.delayDefinitionShoot = 1  # parent decides delay for shoot laterals   |\label{l2_1_2:ddShootB}|
...
rrp.ldelay  = 2.5  # mean lateral delay [days]   |\label{l2_1_2:root_ldelayB}|
rrp.ldelays = 0.   # std dev of delay  [days]    |\label{l2_1_2:root_ldelaysB}|


# Stem subType 1 (main stem): shoot laterals wait 5 days.
srp = plant.getOrganRandomParameter(pb.stem)[1]
srp.ldelay  = 2.5   # |\label{l2_1_2:stem_ldelayB}|
srp.ldelays = 0.    # |\label{l2_1_2:stem_ldelayB}|
...
srp = plant.getOrganRandomParameter(pb.seed)[0]
srp.delayDefinition      = 0  # roots: use distance-based delay       |\label{l2_1_2:ddRootC}|
srp.delayDefinitionShoot = 2  # shoot: lateral decides its own delay       |\label{l2_1_2:ddShootC}|



# Leaf subType 1: shoot laterals wait 5 days.
lrp = plant.getOrganRandomParameter(pb.leaf)[1]
lrp.ldelay  = 2.5  # |\label{l2_1_2:leaf_ldelayC}|
lrp.ldelays = 0.   # |\label{l2_1_2:leaf_ldelaysC}|
