...
for i in range(0, len(treatment_)):
    for j in range(0, len(DAS_)):
        ...
        xind = int(i/2)
        yind = int(i%2!=0)
        ax[xind,yind].plot(x,depth_, color = cols[j], linestyle = linst[0])
        ax[xind,yind].plot(x2,depth2_-10, color = cols[j], linestyle = linst[1])
...
for j in range(0, len(DAS_)):
    plt.plot(np.nan,np.nan, color = cols[j], label = BBCH_[j])
