...
for i in range(0, len(treatment_)):
    for j in range(0, len(DAS_)):
        for k in range(0, len(depth_)): 
            data_ = df[(df['treatment']==treatment_[i])&(df['growth.stage']==BBCH_[j])&(df['depth']==depth_[k])&(df['year']==2019)&(df['position']=='UP')]
...
for i in range(0, len(treatment_)):
    for j in range(0, len(DAS_)):
        ...
        xind = int(j/2)
        yind = int(j%2!=0)
        ax[xind,yind].plot(x,depth_, color = cols[i], linestyle = linst[0])
        ax[xind,yind].plot(x2,depth2_-10, color = cols[i],linestyle = linst[1])
...
plt.plot(np.nan,np.nan, color = 'k', linestyle = linst[0], label = 'Maxime')
plt.plot(np.nan,np.nan, color = 'k', linestyle = linst[1], label = 'Helena')
