...
for i in range(0, len(treatment_)):
    for j in range(0, len(DAS_)):
        ...
        for k in range(0, len(depth_)):
            x = data[i,j,k] #Maxime
            ax[xind,yind].plot([x,x],[depth_[k]-2.5,depth_[k]+2.5], color = cols[i], linestyle = linst[j],linewidth = 2)

        for k in range(0, len(depth2_)): 
            x2 = data2[i,j,k] #Helena
            ax[xind,yind].plot([x2,x2],[depth2_[k]-20, depth2_[k]], color = cols[i], linestyle = linst[j],linewidth = 0.8)
        ...
        ax[xind,yind].set_title(treatment_[i], color = cols[i])
