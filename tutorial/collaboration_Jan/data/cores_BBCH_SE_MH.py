...
for i in range(0, len(treatment_)):
    for j in range(0, len(DAS_)):
        ...
        for k in range(0, len(depth_)):
            x = data[i,j,k] #Maxime
            error = data_SE[i,j,k] #Maxime
            ax[xind,yind].plot([x,x],[depth_[k]-2.5,depth_[k]+2.5], color = cols[j],linewidth = 2)
            ax[xind,yind].fill_betweenx([depth_[k]-2.5,depth_[k]+2.5], [x-error,x-error], [x+error,x+error], color = cols[j],alpha = 0.3)

        for k in range(0, len(depth2_)): 
            x2 = data2[i,j,k] #Helena
            error2 = data2_SE[i,j,k] #Helena
            ax[xind,yind].plot([x2,x2],[depth2_[k]-20, depth2_[k]], color = cols[j],linewidth = 0.8)
            ax[xind,yind].fill_betweenx([depth2_[k]-20,depth2_[k]], [x2-error2,x2-error2], [x2+error2,x2+error2], color = cols[j],alpha = 0.3)
...
for j in range(0, len(DAS_)):
    ax[0,1].plot(np.nan,np.nan, color = cols[j], label = BBCH_[j])
ax[0,1].plot(np.nan,np.nan, color = 'k', linewidth = 2, label = 'Maxime')
ax[0,1].plot(np.nan,np.nan, color = 'k', linewidth = .8, label = 'Helena')
