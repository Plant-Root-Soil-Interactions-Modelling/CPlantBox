...
for i in range(0, len(treatment_)):
    for j in range(0, len(DAS_)):
        ...
        for k in range(0, len(depth_)):
            ...
            ax[xind,yind].fill_betweenx([depth_[k]-2.5,depth_[k]+2.5], [x-error,x-error], [x+error,x+error], color = cols[i],alpha = 0.3)

        for k in range(0, len(depth2_)): 
            x2 = data2[i,j,k] #Helena
            error2 = data2_SE[i,j,k]
            ax[xind,yind].plot([x2,x2],[depth2_[k]-20, depth2_[k]], color = cols[i],linewidth = 0.8)
            ax[xind,yind].fill_betweenx([depth2_[k]-20,depth2_[k]], [x2-error2,x2-error2], [x2+error2,x2+error2], color = cols[i],alpha = 0.3)
