...
marker = ['x', '.']
alts = ['base','alt']
labels = [['75x12 cm', '50x20 cm'],['21x1.5 cm', '12x3 cm']]
annot = ['(g)', '(h)']
...
for i in range(0,len(df_)): 
    ...
    for k in range(0, len(alts)): 
        ...
        if i == 0: 
            df = pd.read_csv("../results/Zea_mays_3_Postma_2011_day120_reps100_imgsize2x2tubediam6.4_pdensity_"+alts[k]+".csv")
        else: 
            df = pd.read_csv("../results/wheat_Morandage_day215_reps100_imgsize2x2tubediam6.4_pdensity_"+alts[k]+".csv")
