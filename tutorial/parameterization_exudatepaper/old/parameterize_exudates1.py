...
mean = df[exudate].values*12*24/10**3
std = df[exudate+' std'].values*12*24/10**3
...
exu_prop1 = 0.000065 #[kg/(m2 day)]
exu_prop2 = 0.000017 #[kg/(m2 day)]
exu_prop3 = 0.0000153 #[kg/(m2 day)]
exu_prop4 = 0.0000115 #[kg/(m2 day)]
dummy = 0
        ...
        for i, s in enumerate(tipI):
            if types[i] == 0:
                ...
                while l_<= kex_[0][1]:
                    ...
                    sf.append(2 * np.pi * a[i] * l[i] * 1.e-4 * kexu * 1.e3) # g/day/root
                    s_ = s_-1
                    if l_>polylengths[i]:
                        #print('here its done')
                        break
