...
for i in range(0, len(soil_type_)):
    ...
    for j in range(0, len(genotype_)):
        ...
        axs[i,j].plot(t_, tpot, 'b',  label = "Potential transpiration")
        axs[i,j].plot(t_, evap,  'r', label = "Potential evaporation")
        axs[i,j].fill_between(x=t_, y1= lai(t_), color= "grey",alpha= 0.3, label = 'LAI')
        axs[i,j].set_xlabel("Time (d)")
        axs[i,j].set_ylabel(r'LAI (-) / Potential evaporation ($mm\ d^{-1}$) /''\n'' Potential transpiration ($mm\ d^{-1}$)')
