...
name = "Zea_mays_5_Leitner_Streuber_2014"
...
def err(fitparams):
    ...
    r = fitparams[0]; ln1=fitparams[1]; ln2 = fitparams[2]; maxB=fitparams[3]; delayB=fitparams[4];  nc=fitparams[5];  lmax1=fitparams[6]; lmax2=fitparams[7];
    ...
    for k in range(0,sims):
        ...
        for j in range(0,times[-1]+1): 
            ...
            if j in times and k == 0:
                rs.writeParameters("rootsystem_params/Optimization_"+soil+"_"+gt+"_test.xml")
                rs.write("rootsystem_params/Optimization_"+soil+"_"+gt+"_day"+str(j)+".vtp")
    ...
    df_virt.to_csv("Length_diam/virtual_length_"+soil+"_"+gt+".csv")


    #compare measured and simulated core RLDs 
    df_meas = pd.read_csv("Length_diam/measured_length_"+soil+"_"+gt+".csv")
...
ln2std = 0.1 #cm
maxBstd = 5
nc = 5 #number of crown roots per whirl 


#p0=[r,ln1,ln1std,ln2,ln2std,maxB,maxBstd,delayB,delayRC,lmax1,lmax2] #initial guess
p0=[r,ln1,ln2,maxB,delayB,nc,lmax1,lmax2] #initial guess
...
set_all_sd(rs) #set the std zero
rs.writeParameters("rootsystem_params/Optimization_"+soil+"_"+gt+".xml")
