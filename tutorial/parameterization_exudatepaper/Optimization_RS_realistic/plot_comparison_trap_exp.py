...
for h in range(0, len(Origin_)): 
    for i in range(0, 1): #len(Substrate_)):
        for j in range(0, len(Location_)):
            ...
            if data1.empty:
                ...
                ax1.bar(offset+h, data1['mean exu rate'], color = cols[i], alpha = alpha[j])
                if h == 0: 
...
ax1.bar(np.nan, np.nan, color = cols[0], label = 'Loam, Tip exudation rate')
ax1.bar(np.nan, np.nan, color = cols[1], label = 'Sand, Tip exudation rate')
ax1.bar(np.nan, np.nan, color = cols[0], alpha = 0.5, label = 'Loam, Base exudation rate')
ax1.bar(np.nan, np.nan, color = cols[1], alpha = 0.5, label = 'Sand, Base exudation rate')
ax1.legend()
#ax1.set_ylim([0,600])
    ...
    for i in range(0, len(Substrate_)):
        for j in range(0, len(Location_)):
            ...
            if data1.empty:
                ...
                offset = width * multiplier
                ax2.bar(offset+h, data1['mean exu rate'], color = cols[i], alpha = alpha[j])
                if h == 0: 
