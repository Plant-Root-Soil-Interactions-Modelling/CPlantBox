...
exu_rates_ = [np.array([0.0015, 0.0015,0.0009,0.00045,0.00044]),
              np.array([0.002, 0.002,0.0006,0.00034,0.00033]),
              np.array([0.0015, 0.0015,0.0009,0.00045,0.00044]),
              np.array([0.002, 0.002,0.0006,0.00034,0.00033])]#[kg/(m2 day)]




plant_exud = np.zeros((154,len(names),3))
    ...
    for j in range(0,154):
        ...
        for i in range(0, len(polylines)):
            ...
            for k in range(0, len(polylines[i])-1):
                ...
                if n.z >-20:
                    sf1.append(2 * np.pi * a * l * 1.e-4 * kexu * 1.e3) # g/day/root
                elif -20>n.z >-40:
                    sf2.append(2 * np.pi * a * l * 1.e-4 * kexu * 1.e3) # g/day/root
                elif -40>n.z >-60:
                    sf3.append(2 * np.pi * a * l * 1.e-4 * kexu * 1.e3) # g/day/root
                    
                surf_.append(2 * np.pi * a * l)

        surf[j,z] = np.sum(surf_) 
        plant_exud[j,z,0] = np.sum(sf1) # g/day/plant
        plant_exud[j,z,1] = np.sum(sf2) # g/day/plant
        plant_exud[j,z,2] = np.sum(sf3) # g/day/plant

    del rs

np.savez('plant_exud_rates_layers', plant_exud)
