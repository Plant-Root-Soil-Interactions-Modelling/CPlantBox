...
for i in range(0,len(soils)):
    for j in range(0, len(gt)):
        ...
        ax[0].plot(y['DAS'], y['Length']/100, color = cols[i+2*j], label = soils[i]+'_'+gt[j])
        ax[0].fill_between(y['DAS'], (y['Length']-y['Length std']/2)/100, (y['Length']+y['Length std']/2)/100, color = cols[i+2*j], alpha = 0.2)
        ax[0].set_ylabel('Total root length\n (m / plant)')

        ax[1].plot(y['DAS'], y['diam'], color = cols[i+2*j], label = soils[i]+'_'+gt[j])
        ax[1].fill_between(y['DAS'], y['diam']-y['diam std']/2, y['diam']+y['diam std']/2, color = cols[i+2*j], alpha = 0.2)
