...
kex = np.array([0.002,0.00065,0.00045,0.00045]) #[kg/(m2 day)]

#import exudate data
exudate = ' Total exudation'
df = pd.read_csv("data/mean_exudation.csv")
DAS = df["DAS"].loc[:].values
mean = df[exudate].values*12*24/10**3 #mmol/plant/h --> g/plant/day
SE = df[exudate+' std'].values/2*12*24/10**3 #std --> SE

#create root system 
rs = pb.MappedRootSystem()
path = "rootsystem/"
name = "RS_optimized_column_S_WT"
