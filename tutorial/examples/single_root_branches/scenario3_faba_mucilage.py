...
for jj in range(0,len(name_)):
    ...
    rs.write("vtp/faba_mucilage/"+name_[jj] +"_day"+("{:02d}".format(ii+1))+".vtp")

    #
    # Grid parameter
    #
    nodes = np.array([np.array(n) for n in rs.getNodes()])
    np.save("nodes/faba_mucilage/"+name_[jj] + "_day"+str(ii+1), nodes)
    xres = 0.1;
    yres = 0.1;
    zres = 0.1;
    nx = int(width / xres);
    ...
    np.save("concentration/faba_mucilage/"+name_[jj] +"_day"+("{:02d}".format(ii+1)), C)

    X = np.linspace(-width / 2, width / 2, nx)
    Y = np.linspace(-width / 2, width / 2, ny)
    Z = np.linspace(-depth, 0, nz)

    gridToVTK("exud/faba_mucilage/./Exudates_"+name_[jj] + "_day"+("{:02d}".format(ii+1)), X, Y, Z, pointData = {"Exudates":C})
