...
initphi =0.
"""
#rx = np.full(len(phiOld),p_linit)

####
#
# xylem -> to get Ccontent init value
#
####
...
weatherD = weatherData(0)
rx = phl.solve_leuning( sim_time = 1,sxx=[p_s], cells = True, Qlight = weatherD['Qlight'] ,VPD = weatherD['VPD'],
...
timeSpanLeuning = 5*60*60  #time in seconds between to evaluation of the gs operture/xylem flow
timeSinceLastLeuning = np.inf

timeSpanPlot = np.inf
timeSinceLastPlot =0.

timeSpanGrowth = 24 *60*60
...
alldts = np.array([])
ana = pb.SegmentAnalyser(phl.rs)
ana.write("results/example10b.vtp" , ["radius", "surface"])
maxdt = 60
increase = 1.5
...
while simDuration < 36*60*60: #
    ...
    if (timeSinceLastLeuning > timeSpanLeuning or step == 0):
        ...
        phl.seg_ind = seg_tips # segment indices for Neumann b.c.
        phl.node_ind = node_tips
        rx = phl.solve_leuning( sim_time = dt,sxx=[p_s], cells = True, Qlight = weatherD['Qlight'] ,VPD = weatherD['VPD'],
            Tl = weatherD['TairK'] ,p_linit = p_linit,ci_init = cs*0.3,cs=cs, soil_k = [], log = False)
