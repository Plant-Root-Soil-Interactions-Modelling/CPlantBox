...
for st in range(1,50):
    ...
    np.save("nodes/maize_citrate/day"+str(ii+1), nodes)
    xres = 0.1;
    yres = 0.1;
    zres = 0.1;
    nx = int(width / xres);
    ...
    np.save("concentration/maize_citrate/day"+("{:02d}".format(ii+1)), C)

    X = np.linspace(-width / 2, width / 2, nx)
    Y = np.linspace(-width / 2, width / 2, ny)
    Z = np.linspace(-depth, 0, nz)

    gridToVTK("exud/maize_citrate/./Exudates_day"+("{:02d}".format(ii+1)), X, Y, Z, pointData = {"Exudates":C})
