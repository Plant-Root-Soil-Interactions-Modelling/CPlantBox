...
for ii in range(0,len(phase)):
    for jj in range(0,len(param)):    
        ...
        with open("Pearson/"+phase[ii]+"_"+param[jj]+"_virt.txt") as f: 
            ...
            print('Pearsons correlation: %.3f' % corr_coeff)
...
for j in range(0,len(vars)): 
    ...
    axs[j+1].bar(np.nan, np.nan, color = 'b', label = 'measurement')
    axs[j+1].bar(np.nan, np.nan, color = 'g', label = 'simulation')
    ...
    axs[j].set_xlim([0,400])
    axs[j+1].set_xlim([0,400])
    ...
    axs[j].text(250,0.5,"R = "+ str(pearsonR[0])+"\nrRMSE = "+str(int(rRMSE[0]))+"%")
    axs[j+1].text(250,0.5,"R = "+ str(pearsonR[1])+"\nrRMSE = "+str(int(rRMSE[1]))+"%")
