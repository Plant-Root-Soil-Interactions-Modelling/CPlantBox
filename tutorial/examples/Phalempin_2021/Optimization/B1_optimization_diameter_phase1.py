...
def err(fitparams):
    ...
    for i in range(0,sims):
        ...
        for j in range(0, len(cores)):
            ...
            #momentary
            #if i == sims: 
            #    core_analyser.write("results_diam/core"+str(j)+".vtp");
            ...
            rm_[j,i] = mean_diam #matrix that contains the mean diameter

    rm = np.nanmean(rm_,axis = 1)
    std = np.nanstd(rm_,axis = 1)
    rmstd = np.concatenate((rm, std))
    np.savetxt("diams/p1_virt_rm_std_"+mat+"_"+den+".txt",rmstd,fmt='%.4f')

    #compare measured and simulated core RLDs 
    with open("diams/p1_measured_diam_std_"+mat+"_"+den+".txt") as f: #read in measured RLDs
...
rs.writeParameters("rootsystem_params/p1_Optimization_diam.xml")
