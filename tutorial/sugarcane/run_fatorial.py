...
for comb in combinations:
    ...
    # read csv results
    df_res_comb = pd.read_csv(Path(args.dest_path).joinpath(f'root_tab_{sim_ID}.csv'), sep=";", decimal=",")
    df_res.append(df_res_comb)

# %%
# concatenate all results and save
df_res = pd.concat(df_res, ignore_index=True)
df_res.to_csv(Path(args.dest_path).joinpath('root_tab_fatorial.csv'), sep=";", decimal=",", index=False)
# %%
