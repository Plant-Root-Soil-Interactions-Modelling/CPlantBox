...
rs_age = 1  # root system initial age
...
npzfile = np.load('sims_root_soil_coarse.npz')
swc =npzfile['arr_0']
rwc =npzfile['arr_1']
cn = npzfile['arr_2']
...
srwc_ = srwc[:,int(cn[1]/2),:]
colors = plt.cm.seismic_r(srwc_)
#norm = matplotlib.colors.Normalize(vmin=np.min(srwc_), vmax=np.max(srwc_))
norm = matplotlib.colors.Normalize(vmin=0, vmax=0.15)
ax4.imshow(srwc_, cmap = 'seismic_r',norm = norm, aspect=1)#1.67
m = cm.ScalarMappable(cmap=plt.cm.seismic_r, norm=norm)
m.set_array([])
plt.colorbar(m, ax = ax4, label = 'Water content (-)')
