...
sand = [0.045, 0.43, 0.15, 3, 1000]    #|\label{l62:params_a}|
loam = [0.08, 0.43, 0.04, 1.6, 50]
clay = [0.1, 0.4, 0.01, 1.1, 10]
soil=loam   # Select soil type for simulation   |\label{l62:params_e}|
sim_time=10;   #|\label{l62:simtime}|
N = 2000
dt = sim_time / N  # time step [days]    |\label{l62:timestep}|
ic=-200                               # |\label{l62:ic}|
evap = -0.1   #cm/d       |\label{l62:evap}|

# Solve the Richards equation using the Python wrapper of dumux-rosi 
s = RichardsWrapper(RichardsSP())     # |\label{l62:initialize_a}|
s.initialize()                       # |\label{l62:initialize_e}|
s.setTopBC("atmospheric", 0.5, [[0., 1.e10], [evap, evap]])  #  [cm/day] atmospheric is with surface run-off   |\label{l62:top_bc}|
# s.setTopBC("flux", evap)  #  [cm/day] 
# s.setTopBC("constantPressure", -10000)  
s.setBotBC("freeDrainage")  # BC freeDrainage   |\label{l61:bot_bc}|
NZ=1399
s.createGrid([-5., -5., -100.], [5., 5., 0.], [1, 1, NZ])  # [cm]   |\label{l62:grid}|
#vols = (100. / NZ) * np.ones((NZ,)) * 100.  # cm3
s.setVGParameters([soil])    #|\label{l62:set_vg}|
s.setHomogeneousIC(ic)  # cm pressure head  |\label{l62:set_ic}|
# s.setParameter("Problem.EnableGravity", "false")
s.initializeProblem()  #|\label{l62:initialise}|
s.setCriticalPressure(-10000)   #|\label{l62:criticalPsi}|
s.setRegularisation(1.e-6, 0.)   #|\label{l62:regularize}|
idx_top = s.pickCell([0.0, 0.0, 0.0])  # index to watch surface flux  #|\label{l62:picker}|
initial_water = s.getWaterVolume()  #|\label{l62:gettheta}|
print(initial_water)
s.ddt = 1.e-5  # initial Dumux time step [days]  |\label{l62:initialDT}|
maxDt = 1.  # maximal Dumux time step [days]   |\label{l62:maxDT}|

x_, y_ = [], []
for i in range(0, N):  # |\label{l62:loop}|
    s.solve(dt, maxDt)  #|\label{l62:solve}|
    f = s.getNeumann(idx_top)  # f = s.getSolutionHeadAt(idx_top)   |\label{l62:Neumann_a}|
    #   current_water = s.getWaterVolume()
    #   f = (initial_water - current_water) / dt / 1.e2
    #   print(current_water, f)
    #   initial_water = current_water
    x_.append(s.simTime)
    y_.append(f)  # |\label{l62:Neumann_e|

# Extract and plot numerical solution  
plt.plot(x_, y_, 'b')  #|\label{l62:plot_fluxes_a}|
...
plt.show()   #|\label{l62:plot_fluxes_e}|