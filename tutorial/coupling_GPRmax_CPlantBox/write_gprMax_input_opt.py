...
def load_cplantbox_input(file):
    ...
    domain_x_y = npzfile['arr_5']  # 'target' from simulation
    soil_height = (npzfile['arr_6']/-100)
    thetas = 0.43
    indices_rvf = np.where(rvf > 0)  # find indices of cells where roots are present
    return swc, rvf, soil_height, thetas, indices_rvf, domain_x_y, grid_res, gprMax_name
...
def get_gprmax_settings():
    ...
    return domain, c_f_Mhz, tw, abc_buffer, bcells, h_air, tx_depths, tx_pos, rx_pos
...
def write_gprmax_input_file(file, swc, rvf, soil_height, thetas, indices_rvf, domain_x_y, grid_res, gprMax_name):
    """Write gprMax input file"""
    domain, c_f_Mhz, tw, abc_buffer, bcells, h_air, tx_depths, tx_pos, rx_pos = get_gprmax_settings()
    ...
    d_x = domain_x_y / 100  # domain size in x-direction
    d_y = domain_x_y / 100  # domain size in y-direction
    d_z = soil_height + h_air  # domain size in z-direction
    tx_x = tx_pos + abc_buffer
    tx_y = d_y
    tx_z = d_z - tx_depths[0]
    rx_x = rx_pos + abc_buffer
    if grid_res == 'high':
        grid_size = 0.01
    else:
        grid_size = 0.03
    ...
    with open(in_filename, 'w') as fid:
        ...
        fid.write(f"#hertzian_dipole: z {tx_x:4.3f} {soil_height:4.3f}  {rx_pos:4.3f} my_ricker\n")
        fid.write(f"#rx: {rx_x:4.3f} {soil_height:4.3f} {rx_pos:4.3f}\n")
        fid.write("#src_steps: 0 -0.1 0\n")
        fid.write("#rx_steps: 0 -0.1 0\n\n")
        fid.write(f"#box: 0 0 0 {tx_x:4.3f} {tx_y:4.3f} {tx_z:4.3f} half_space  n\n")
        # Write boxes for each cell
        root_x, root_y, root_z = eps_ss.shape
        for k in range(root_z):
            for j in range(root_y):
                for i in range(root_x):
                    # Find index where the value in eps_bulk_root matches
                    match = np.where(eps_ss_uniq == eps_ss[i, j, k])[0]
                    if match.size > 0:
                        idx = int(match[0]) + 1
                        root_soil_field = f'root_soil_{idx}'
                        x0 = 0.01 * j + abc_buffer
                        y0 = 0.01 * i + abc_buffer
                        z0 = 0.01 * k + abc_buffer
                        x1 = 0.01 * (j + 1) + abc_buffer
                        y1 = 0.01 * (i + 1) + abc_buffer
                        z1 = 0.01 * (k + 1) + abc_buffer
                        fid.write(f"#box: {x0:4.3f} {y0:4.3f} {z0:4.3f} {x1:4.3f} {y1:4.3f} {z1:4.3f} {root_soil_field}  n\n")
        fid.write("\n")
...
if __name__ == "__main__":
    ...
    for file in out_files:
        ...
        swc, rvf, soil_height, thetas, indices_rvf, domain_x_y, grid_res, gprMax_name = load_cplantbox_input(file_path)
        write_gprmax_input_file(file_path, swc, rvf, soil_height, thetas, indices_rvf, domain_x_y, grid_res, gprMax_name)