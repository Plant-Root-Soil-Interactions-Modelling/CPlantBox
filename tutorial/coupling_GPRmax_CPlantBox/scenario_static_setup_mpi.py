...
def set_scenario(plant_, res, soil_, initial_, trans_, rs_age, inf_:bool, evap_:bool):
    ...
    target_x = 175 #cm, length of target domain for gpr max in x
    target_y = 175 #cm, length of target domain for gpr max in y
    soil_depth = -150 
    ...
    # plant.write('results/'+plant_+'.vtp')
    hm.test()
    
    return wilting_point, soil, s, hm, plant, target_x,target_y, cell_number, cellvol, X, Y, Z, wc_root, res_ , soil_depth, soil_
...
def write_vtr(name, t, target_x, target_y, X, Y, Z, wc_root, wc, hs, rootvol, wc_stitch, hs_stitch, rootvol_stitch, plant, res, soil_depth): 
    ...
    X_stitch = np.linspace(-target_x/2, target_x/2, cell_number_stitch[0])
    Y_stitch = np.linspace(-target_y/2, target_y/2, cell_number_stitch[1])
    Z_stitch = np.linspace(soil_depth, 0, cell_number_stitch[2])
    ...
    gridToVTK('results/'+directory+'/'+name+'_stitch_day_'+str(int(t)), X_stitch, Y_stitch, Z_stitch, pointData = {"Water content":np.array(wc_stitch),"Soil water potential":np.array(hs_stitch),"Root volume":np.array(rootvol_stitch), "water content root and soil":np.array(wc_root_soil_stitch)})