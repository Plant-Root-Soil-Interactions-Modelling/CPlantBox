...
def main():
    ...
    soil_core_1, soil_core_2, _, _, _ = define_soil_core_geometry(
        dataset
    )
    ...
    for plant_index in range(n_plants_per_row):
        for row_index in range(n_rows):
            ...
            run_daily_growth_simulation(root_system, soil_temp_df, scale_elongation, dataset)
    ...
    soil_cores = pb.SDF_Union(soil_core_1_shifted, soil_core_2_shifted)
    ...
    obstacle_pd = sdf_to_vtk_mesh_custom(
        soil_cores, field_bounds, resolution=80
    )
