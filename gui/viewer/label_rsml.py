...
def label_file(file, z_shift, artificial_shoot, split, scenario_index):
    ...
    if split:
        ...
        for i in ysorted:
            ...
            suf_[i, :] = data.analyser.distribution("SUF", 0.0, float(-n2), int(n2) * 10, False)  # mm layers
        ...
        suf_[0, :] = data.analyser.distribution("SUF", 0.0, float(-n), int(n) * 10, False)  # mm layers
