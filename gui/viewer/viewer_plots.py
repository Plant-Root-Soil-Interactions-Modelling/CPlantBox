...
def plot_suf(data, ax3, j):
    ...
    z_ = np.linspace(-0.5, -n + 0.5, n)
    d = data.analyser.distribution("SUF", 0.0, float(-n), int(n), False)  # False!!!
...
def plot_krs(data, ax, j):
    ...
    ax.clear()
    if j == 0:
        viewer_conductivities.init_constant_scenario1(data.hydraulic_params)
    elif j == 1:
        viewer_conductivities.init_constant_scenario2(data.hydraulic_params)
    elif j == 2:
        viewer_conductivities.init_dynamic_scenario1(data.hydraulic_params)
    elif j == 3:
        viewer_conductivities.init_dynamic_scenario2(data.hydraulic_params)
    elif j == 4:
        viewer_conductivities.init_constant_scenario_wine(data.hydraulic_params)

    t = int(np.ceil(data.max_ct))
    t_ = np.linspace(1, t, t)
    krs_ = []
    for t in t_:
        krs, _ = data.hydraulic_model.get_krs(t)
        krs_.append(krs)
    ax.plot(t_, krs_)
