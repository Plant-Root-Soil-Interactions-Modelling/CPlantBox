...
class ViewerDataModel(RsmlData):
    ...
    * can interpolate creation time  DataModel.add_creation_times
    ...
    def __init__(self):
        ...
        self.hydraulic_model = None  # created by convert_to_xylem_flux_
    ...
    def convert_to_xylem_flux_(self):
        ...
        self.hydraulic_model.neumann_ind = self.base_nodes  # needed for suf
        self.hydraulic_model.dirichlet_ind = self.base_nodes  # needed for krs
        self.base_segs = self.find_base_segments()  # unused?
        self.ct_max = np.max(segCTs)
    ...
    def find_base_segments(self):
        ...
        """
        s_ = []
        segs = self.hydraulic_model.ms.segments
        for i, s in enumerate(segs):
            if s.x in self.hydraulic_model.dirichlet_ind:
                s_.append(i)
        print("ViewerDataModel.find_base_segments(): base segment indices for node indics", self.hydraulic_model.dirichlet_ind, "are", s_)
        return s_
    ...
    def add_creation_times(self):
        ...
        for i, pl in enumerate(pl_):
            if self.properties["parent-poly"][i] == -1:
                self.functions["creation_time"][i] = np.zeros(
                    (
                        len(
                            pl,
                        )
