...
class App:
    ...
    def plot_conductivities(self):
        ...
        if self.data.exists():
            ...
            r = self.data.hydraulic_model  # rename
            if j == 0:
                viewer_conductivities.init_constant_scenario1(self.data.hydraulic_params)
            elif j == 1:
                viewer_conductivities.init_constant_scenario2(self.data.hydraulic_params)
            elif j == 2:
                viewer_conductivities.init_dynamic_scenario1(self.data.hydraulic_params)
            elif j == 3:
                viewer_conductivities.init_dynamic_scenario2(self.data.hydraulic_params)
            elif j == 4:
    ...
    def file_open(self):
        ...
        global fname
        fname = tkinter.filedialog.askopenfilename(title="Please select a RSML root system", filetypes=[("Image Files", [".rsml", ".RSML", ".xml"])])
    ...
    def file_save(self):
        ...
        if self.data.exists():
            ...
            if isinstance(fname, str):
                if fname:
                    pd = vp.segs_to_polydata(self.data.analyser, zoom_factor=1.0, param_names=["subType", "radius", "creationTime"])
    ...
    def view_vtk_plot_multiple_suf(self):
        ...
        if len(self.data.base_segs) == len(self.data.base_nodes):
            ...
            for i in range(0, n):
                ...
                krs[i], _ = r.get_krs(self.data.max_ct)
