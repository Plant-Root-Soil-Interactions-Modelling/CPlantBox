...
def run_benchmark(params, llambdao, kko,genotype = 'B', getdict = False): 
    start_time = time.time()
    output = []
    for rep in range(1):#range(8):
        outputs_12 = {
                'num':[0.,0.,0.,0.,0.],
                'length':[0.,0.,0.,0.],
                #'ratio':0
        ...
        yr = [1,2,50]        
        outpouts_mean = {
            'year'+str(yr[i]): copy.deepcopy(outputs_12) if i < 2 else copy.deepcopy(outputs_50)
            for i in range(3)
        }
        ...
        params['successorP11'] = 0 #1. - params['successorP10']
        for ii, pp in enumerate(plant.getOrganRandomParameter(2)):
            ...
            if ii == 0:      
                ...
                pp.successorP = [[1], [0.1]]#[[params['successorP0']]]
                pp.successorNo = [1] #[params['successorNo0']] 
                pp.successorST = [[3]]
                ...
                pp.ldelay  = 0.#yr_to_BEDD
                pp.ldelays = yr_to_BEDD* 0.5#yr_to_BEDD * params['ldelays0'] #200#*5
                pp.successorP = [[0.7], [0.1]]#[[params['successorP0']]]
                pp.successorNo = [1] #[params['successorNo0']] 
                pp.successorST = [[4]]
                pp.successorP_age = [1225*3, 1225 * 100]
                # pp.r *= 2
                ...
                pp.ldelay  = 0*yr_to_BEDD
                pp.ldelays =yr_to_BEDD*0.5 #yr_to_BEDD * params['ldelays0'] #200#*5
                pp.successorP = [[0.], [0.0]]#[[params['successorP0']]]
                pp.successorNo = [1] #[params['successorNo0']] 
                pp.successorST = []
                #pp.k_survive = 1.261
                #pp.lambda_survive = 1.367
        ...
        dt = yr_to_BEDD  # ~1 yr
        N = 50
        dates = np.diff([0, 1,2,50])

        # get3Dshape(plant,title_ = 'wine0', saveOnly = True)
        for i in range(N):#dates:#
            print('age',i, len(all_alive),end=", ")
            plant.survivalTest()
        ...
        for year in [N - 1]:#range(N):
            '''
            Num
            '''
            # TODO: only select roots that could be excavated
            
            for st in range(subtypes): # from 2 to 5
                value = sum(all_alive[2][all_subtypes[2] == (st + 1)])
                outpouts_mean['year'+str(year+1)]['num'][st] = value  
                # value = sum(all_alive[year][all_subtypes[year] == (st + 1)])
                # outpouts_mean['year'+str(year+1)]['num'][st] = value   
        ...
        def flatten_values(obj):
            if isinstance(obj, dict):
                ...
                yield obj
        output.append(list(flatten_values(outpouts_mean)))
...
if __name__ == '__main__': 
    ...
    print(run_benchmark(getDefaultFile(), genotype = 'B', getdict = True, llambdao = 4.995, kko = 1.367))