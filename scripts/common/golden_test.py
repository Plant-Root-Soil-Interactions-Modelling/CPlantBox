...
def _prefer_installed_wheel() -> None:
    ...
    if platlib:
        sys.path[:] = [platlib] + [
            p
            for p in sys.path
            if p != platlib and not p.startswith("/project") and not p.startswith("/src")
        ]


def _append_test_path() -> None:
...
def main() -> int:
    ...
    # Select golden OS subdir: linux (default) or macos
    golden_os = os.environ.get("CPB_GOLDEN_OS")
    if not golden_os:
        golden_os = "macos" if sys.platform == "darwin" else "linux"

    _prefer_installed_wheel()
    ...
    # Parameters packaged inside the wheel
    ...
    for root in (os.getcwd(), "/src", "/project"):
        if not os.path.isdir(root):
            continue
        dst = os.path.join(root, "test", "golden", "generated_by_test.png")
        try:
    ...
    golden_dir = os.path.join(root, "test", "golden", golden_os)
    golden_src = os.path.join(golden_dir, "example_plant_headless.png")
    ref_png = os.path.join(root, "test", "golden", "example_plant_headless.png")
    if not os.path.exists(golden_src):
        print("Golden image missing:", golden_src)
        return 2
    try:
        os.makedirs(os.path.dirname(ref_png), exist_ok=True)
        with open(golden_src, "rb") as fsrc, open(ref_png, "wb") as fdst:
            fdst.write(fsrc.read())
    ...
    try:
        thresh = float(os.environ.get("CPB_GOLDEN_THRESH", "0.9"))
    except Exception:
        thresh = 0.9

    sim = compare_images_png(out_png, ref_png)
    print("golden_similarity=", sim)
    return 0 if sim >= thresh else 2
